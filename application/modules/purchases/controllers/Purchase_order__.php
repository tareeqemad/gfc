...
class Purchase_order extends MY_Controller{
    ...
    function index($page= 1, $committees= -1, $committee_case= -1, $purchase_order_id= -1, $purchase_order_num= -1, $purchase_type= -1, $purchase_order_class_type= -1, $notes= -1, $entry_user= -1, $adopt= -1, $design_quote_user= -1, $design_quote_case= -1, $delay_case= -1, $convert_case= -1,$from_date=-1,$to_date=-1 ){
        $this->_look_ups($data);
        if($this->order_purpose!=1 and $this->order_purpose!=2)
            die('index');

        if($this->quote==1)
            $data['title']='عرض الاسعار';
        elseif($committees==1 and $committee_case==2)
            $data['title']='محاضر لجنة فتح المظاريف';
        elseif($committees==1 and $committee_case==4)
            $data['title']='محاضر لجنة البت والترسية';
        elseif($this->order_purpose==2)
            $data['title']='طلبات الاعمال المدنية';
        else
    ...
    function get_page($page = 1, $committees= -1, $committee_case= -1, $purchase_order_id= -1, $purchase_order_num= -1, $purchase_type= -1, $purchase_order_class_type= -1, $notes= -1, $entry_user= -1, $adopt= -1, $design_quote_user= -1, $design_quote_case= -1, $delay_case= -1, $convert_case= -1 ,$from_date=-1,$to_date=-1){
        ...
        if($this->quote==1)
            $where_sql.= " and purchase_type in (2,3) and adopt=70 ";
        if(!HaveAccess(base_url($this->ALL_BRANCHES)))
        ...
        if( !$this->input->is_ajax_request() and $committees== null and $this->quote!=1){
            ...
            if(HaveAccess(base_url("purchases/purchase_order/create?order_purpose={$this->order_purpose}")))
                $adopt_where_sql= " and adopt= 10 and entry_user= {$this->user->id} ";
            $ADOPT_VAL_PRIV=0;
         ...
         //   print_r(substr($priv[0]['PR'],-2));
           ...
           /* for($i=15;$i<=70;$i+5){
            ...
            if ($ADOPT_VAL_PRIV==40 || ($this->user->id==477)||($this->user->id==116))
            $adopt_where_sql.= " and purchase_pkg.ASSISTENT_MANAGER(PURCHASE_ORDER_ID)=1 ";
...
//echo $where_sql;
    ...
    function public_get_page($text= null, $page = 1, $committees= -1, $committee_case= -1, $purchase_order_id= -1, $purchase_order_num= -1, $notes= -1){
        ...
        $where_sql= " where order_purpose='{$this->order_purpose}' ";
        if($this->quote==1)
            $where_sql.= " and purchase_type in (2,3) and design_quote_date is not null ";
        if(!HaveAccess(base_url($this->ALL_BRANCHES)))
            $where_sql.= " and branch= {$this->user->branch} ";
    ...
    function _post_validation($isEdit = false){
        ...
        }else if( $this->order_purpose==1 and ( !($this->class_id) or count(array_filter($this->class_id)) <= 0 or count(array_filter($this->amount)) <= 0 ) ){
            $this->print_error('يجب ادخال صنف واحد على الاقل ');
        }else if( $this->order_purpose==2 and ( !($this->item) or count(array_filter($this->item)) <= 0 or count(array_filter($this->amount)) <= 0 ) ){
        ...
        if ($this->p_purchase_order_class_type==11){
            ...
            for($i=0;$i<count($this->item);$i++){
                ...
                if($this->amount[$i]!='' and $this->item[$i]=='' )
                    $this->print_error('ادخل البند');
                if($this->amount[$i]!='' and $this->class_unit[$i]=='' )
                    $this->print_error('اختر الوحدة');
                elseif($this->amount[$i]!='' and ($this->buy_price[$i]=='' or $this->buy_price[$i]<=0) )
                    $this->print_error('ادخل السعر');
            }
    ...
    function get($id, $action= 'index'){
        ...
        if(!(count($result)==1 and (HaveAccess(base_url($this->ALL_BRANCHES)) or $result[0]['BRANCH']== $this->user->branch) and ($this->quote==0 or ($this->quote==1 and $result[0]['ADOPT']==70) ) ))
            die('get');
        ...
        $data['next_adopt_email']= $this->get_emails_by_code('3.'.($next_adopt[$result[0]['ADOPT']])); // old 202003 - ($result[0]['ADOPT']+20)
        ...
        if($result[0]['ADOPT']==40 and $result[0]['TOTAL_COST']<3001){
            $data['next_adopt_email']= $this->get_emails_by_code('3.60');
        }
    ...
    function public_get_det_items($id= 0, $adopt= 0, $quote=0,$civil_type){
        ...
        $quote = $this->input->post('quote') ? $this->input->post('quote') : $quote;
        $civil_type = $this->input->post('civil_type') ? $this->input->post('civil_type') : $civil_type;
         ...
         $data['details'] = $this->{$this->DET_ITEMS_MODEL_NAME}->get_list($id);
    ...
    function public_get_section_data(){
        ...
        if(count($section_data)==1 || 1)
            $this->return_json($section_data[0]);
        else
    ...
    function adopt_45(){ // new - 07/05/2020
        if ($_SERVER['REQUEST_METHOD'] == 'POST' and $this->purchase_order_id!=''){
            echo $this->adopt(45);
        }else
            echo "لم يتم ارسال رقم الطلب";
    ...
    function _postedData_quote(){
        $result = array(
            array('name'=>'PURCHASE_ORDER_ID','value'=>$this->purchase_order_id ,'type'=>'','length'=>-1),
            array('name'=>'QUOTE_CURR_ID','value'=>$this->quote_curr_id ,'type'=>'','length'=>-1),
            array('name'=>'QUOTE_START_DATE','value'=>$this->quote_start_date ,'type'=>'','length'=>-1),
            array('name'=>'QUOTE_END_DATE','value'=>$this->quote_end_date ,'type'=>'','length'=>-1),
            array('name'=>'QUOTE_CONDITION','value'=>$this->quote_condition ,'type'=>'','length'=>-1),
        );
        return $result;
    }
    ...
    function _postedData_details($ser= null, $class_id, $amount= null, $buy_price= null, $note= null, $order_colum= null,$section_no= null, $typ= null){
        $result = array(
            ...
            array('name'=>'PRICE','value'=>str_replace(",","",$buy_price) ,'type'=>'','length'=>-1),
        ...
        if($typ=='create')
            unset($result[0]);
        if($typ=='approved')
            unset($result[2],$result[4],$result[5],$result[7]);
        return $result;
    }
    ...
    function public_get_group_delay($purchase_order_id,$delay_id=0){
     ...
     if (intval($delay_id)>0)
         $data['rec_groups'] = $this->receipt_class_input_group_model->get_details_all($delay_id,4);
     else
    ...
    function  _postGroupsDataEnvelope($ser ,$id,$group_person_id,$group_person_date,$emp_no,$status,$member_note,$ty =null){
        $result = array(
            ...
            array('name'=>'SOURCE','value'=>3,'type'=>'','length'=>-1),
    ...
    function  _postGroupsDataDelay($ser ,$id,$group_person_id,$group_person_date,$emp_no,$status,$member_note,$ty =null){
        $result = array(
            ...
            array('name'=>'GROUP_PERSON_DATE','value'=>$group_person_date,'type'=>'','length'=>-1),
            array('name'=>'EMP_NO','value'=>$emp_no,'type'=>'','length'=>-1),
            array('name'=>'SOURCE','value'=>4,'type'=>'','length'=>-1),
            array('name'=>'STATUS','value'=>$status,'type'=>1,'length'=>-1),
            array('name'=>'MEMBER_NOTE','value'=>$member_note,'type'=>1,'length'=>-1)
...
//PRINT_R($this->award_notes);
    ...
    function _postedDataDelay($typ= null){
        $result = array(
            ...
            array('name'=>'AWARD_NOTES','value'=>$this->p_award_notess ,'type'=>'','length'=>-1)
        );
        if($typ=='create')
            unset($result[0]);
    ...
    function edit_delay1(){
        ...
        if (intval($this->p_delay_id)>0){
            ...
            // -------
            if(intval($this->p_delay_id) <= 0)
                $this->print_error($this->p_delay_id."فشل في حفظ التعديل ");
            // -------
...
//echo $this->award_hints[$row['SUPPLIERS_OFFERS_ID']][$row['CLASS_ID']]."d";
         ...
         // $this->print_error($x);
           ...
           if (!in_array(3,$this->p_award_delay_decision))
               $this->print_error("عليك تنفيذ أصناف أولا");

            $this->p_delay_id=$this->suppliers_offers_delay_model->create($this->_postedDataDelay('create'));
            // -------
            if(intval($this->p_delay_id) <= 0)
                $this->print_error($this->p_delay_id."فشل في حفظ طلب تأجيل");
            // -------
    ...
    function get_page_sms($page = 1, $purchase_type= -1 ){
       ...
       // $purchase_type= (isset ($purchase_type) && $purchase_type !='') ? $this->check_vars($purchase_type,'purchase_type') : array();
        ...
        for ($i=0; $i<count($purchase_type);$i++){
            ...
            if ($purchase_type[$i]!= 0)
            $purchase_type_t=$purchase_type_t.",".$purchase_type[$i];
        }
