...
class Receipt_logistical_contracts extends MY_Controller
{

    var $MODEL_NAME = 'Receipt_logistical_contracts_model';
    var $MODEL_NAME_DET = 'Receipt_logistical_detail_model';
    var $PAGE_URL = 'purchases/Receipt_logistical_contracts/get_page';


    //var $MODEL_NAME_GROUP = 'Receipt_logistical_group_model';

    function __construct()
    {
        parent::__construct();
        $this->load->model('Receipt_logistical_contracts_model');
        $this->load->model('Receipt_logistical_detail_model');
        $this->load->model('Receipt_logistical_group_model');
    ...
    function index()
    {
        $data['title'] = ' محاضر الفحص و الإستلام للخدمات اللوجستية';
        $data['help'] = $this->help;
        $data['content'] = 'receipt_logistical_contracts_index';
    ...
    function get_page($page = 1)
    {
        ...
        $where_sql = " where RECEIPT_CLASS_INPUT_ID in (select RECEIPT_CLASS_INPUT_ID_SER from gfc.receipt_logistic_group_tb t WHERE EMP_NO=USER_PKG.GET_USER_EMP_NO)";
        ...
        $count_rs = $this->get_table_count("RECEIPT_LOGISTIC_CONTRACTS_TB", $where_sql);
        ...
        $data['page'] = $page;
        $this->_look_ups($data);
        $this->load->view('receipt_logistical_contracts_page', $data);
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
         ...
         /*  if ((count($this->p_group_person_id) - $member_absants) < 5) {
                ...
                for ($i = 0; $i < count($this->p_amount); $i++) {
                    ...
                    if ($this->p_h_ser[$i] <= 0)
                        $detail_seq = $this->Receipt_logistical_detail_model->create($this->_postedData_details($this->p_h_ser[$i], $result, $this->p_h_item[$i], $this->p_amount[$i], $this->p_customer_type[$i], $this->p_customer_accounts_id[$i], $this->p_item_recipt_no[$i], 'create'));
                    else
                        $detail_seq = $this->Receipt_logistical_detail_model->edit($this->_postedData_details($this->p_h_ser[$i], $result, $this->p_h_item[$i], $this->p_amount[$i], $this->p_customer_type[$i], $this->p_customer_accounts_id[$i], $this->p_item_recipt_no[$i], 'edit'));
                ...
                for ($i = 0; $i < count($this->p_group_person_id); $i++) {
                    ...
                    if ($this->p_h_group_ser[$i] <= 0)
                        $detail_group_seq = $this->Receipt_logistical_group_model->create($this->_postedData_group_details($this->p_h_group_ser[$i], $result, $this->p_group_person_id[$i], $this->p_group_person_date[$i], $this->p_emp_no[$i], $status, $this->p_member_note[$i], 'create'));
                    else
                        $detail_group_seq = $this->Receipt_logistical_group_model->edit($this->_postedData_group_details($this->p_h_group_ser[$i], $result, $this->p_group_person_id[$i], $this->p_group_person_date[$i], $this->p_emp_no[$i], $status, $this->p_member_note[$i], 'edit'));
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            /*if ((count($this->p_group_person_id) - $member_absants) < 5) {
                ...
                for ($i = 0; $i < count($this->p_amount); $i++) {
                    ...
                    $detail_seq = $this->Receipt_logistical_detail_model->create($this->_postedData_details($this->p_h_ser[$i], $result_ser, $this->p_h_item[$i], $this->p_amount[$i], $this->p_customer_type[$i], $this->p_customer_accounts_id[$i], $this->p_item_recipt_no[$i], 'create'));
                ...
                for ($i = 0; $i < count($this->p_group_person_id); $i++) {
                    ...
                    $status = (isset($this->p_status[$i + 1])) ? 1 : 2; // $this->input->post('checkbox',TRUE)==null ? 1 : 2;
                    $detail_group_seq = $this->Receipt_logistical_group_model->create($this->_postedData_group_details($this->p_h_group_ser[$i], $result_ser, $this->p_group_person_id[$i], $this->p_group_person_date[$i], $this->p_emp_no[$i], $status, $this->p_member_note[$i], 'create'));
            ...
            $data['content'] = 'receipt_logistical_contracts_show';
            $data['title'] = 'ادخال محاضر الفحص و الاستلام للخدمات اللوجستية';
            $data['isCreate'] = true;
            $data['action'] = 'index';
            $data['page'] = 1;
            $data['help'] = $this->help;
    ...
    function public_items_recept($id = 0, $class_type = 0)
    {
        ...
        $data['rec_logistic_details'] = $this->Receipt_logistical_detail_model->get($id);
        ...
        $data['rec_ser'] = $id;
        $this->_look_ups($data);
        $this->load->view('receipt_logistical_contracts_detail_page', $data);
    ...
    function get($id)
    {
        ...
        $data['result'] = $result;
        $data['content'] = 'receipt_logistical_contracts_show';
        $data['title'] = ' محاضر الفحص و الاستلام لتعاقدات اللوجستية ';
        $data['action'] = 'edit';
        $data['isCreate'] = false;
        $data['help'] = $this->help;
        $this->_look_ups($data);
    ...
    function public_group_recept($id = 0)
    {
        ...
        $data['rec_ser'] = $id;
        $data['rec_groups'] = $this->Receipt_logistical_group_model->get_group_list($id);
        $data['group_rec_logistic_details'] = $this->Receipt_logistical_group_model->get($id);
        $data['EMP_NO'] = $this->user->emp_no;
        $this->load->view('receipt_logistical_contracts_group_page', $data);
    ...
    function record_record_member()
    {
        ...
        for ($c = 0; $c < count($this->p_group_person_id); $c++) {
            if ($this->p_emp_no[$c] == $data['EMP_NO']) {
                $result = $this->Receipt_logistical_group_model->member_record($this->p_receipt_class_input_id,$this->p_committees_id, $this->p_member_note[$c]);
            }
        }
...
//////////////////////////////////////////
    ...
    ////////////////////////////////////////////
    function Receipt_logistical_adopt()
    {

        $data['title'] = '  اعتماد محاضر الفحص و الاستلام للخدمات اللوجستية ';
        $data['help'] = $this->help;
        $data['content'] = 'receipt_logistical_contracts_adopt_index';
        add_css('combotree.css');
        add_css('jquery.dataTables.css');
        add_js('jquery.dataTables.js');
        add_css('select2_metro_rtl.css');
    ...
    {
        $data['committee_emails'] = $this->Receipt_logistical_group_model->get_committee_emails($id);
