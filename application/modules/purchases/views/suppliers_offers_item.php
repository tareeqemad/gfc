    ...
    <table class="table" id="suppliers_offers_detTbl" data-container="container">
        ...
        <?php foreach($rec_details as $row) :?>
            ...
            <tr>
                ...
                <td>
                    <input type="hidden" name="purchase_ser[]" data-val="true"   id="purchase_ser_<?= $count ?>"   value="<?= $row['PURCHASE_SER'] ?>"  class="form-control col-sm-2">
                ...
                <td> <input type="hidden" name="purchase_amount[]"  disabled  value="<?= $row['PURCHASE_AMOUNT'] ?>" id="purchase_amount_<?= $count ?>"   data-val-required="حقل مطلوب"   data-val-regex="المدخل غير صحيح!"   class="form-control">
                    <?= $row['PURCHASE_AMOUNT'] ?>
                </td>
                <td><input name="amount[]" max="<?= $row['PURCHASE_AMOUNT'] ?>"    id="amount_<?= $count ?>" value="<?= $row['AMOUNT'] ?>"  data-val-required="حقل مطلوب"   data-val-regex="المدخل غير صحيح!"   class="form-control">

                </td>

                <td><input name="customer_price[]"   id="customer_price_<?= $count ?>"      class="form-control" value="<?= $row['CUSTOMER_PRICE'] ?>">

                </td>
                <td><input name="price[]"    id="price_<?= $count ?>"    class="form-control" value="<?= $row['PRICE'] ?>">

                </td>
                <td><input readonly name="total_price[]"    id="total_price_<?= $count ?>"   class="form-control" value="<?= ($row['PRICE']* $row['AMOUNT']) ?>">


                </td >
                <td >
                    <textarea rows="2" name="note[]" id="note_<?= $count ?>" class="form-control" ><?=$row['CLASS_HINTS']?></textarea>
                    <!--   <input name="note[]"  type="text" id="note_<?= $count ?>" value="<?= $row['CLASS_HINTS']?>" class="form-control">-->
