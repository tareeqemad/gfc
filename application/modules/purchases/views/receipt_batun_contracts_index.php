...
$send_to_adopt=base_url("$MODULE_NAME/$TB_NAME/send_to_adopt");
    ...
    <div class="row">
        <div class="toolbar">
            ...
            <ul>
                ...
                <?php  if(HaveAccess($send_to_adopt)) echo "<li><a onclick='javascript:send_to_adopt();' href='javascript:;'><i class='glyphicon glyphicon-download'></i> ترحيل للإعتماد</a> </li>"; ?>
...
x=x+','+ val[v];
        ...
        if(val.length >= 1){
            if(confirm('هل تريد بالتأكيد ترحيل '+val.length+'  المحاضر للإعتماد ؟!!')){
/////////////////
                    get_data('{$send_to_adopt}',{id:x,first_val:first_val },function(data){
                    if(Number(data)>0 )
                         
                          setTimeout(function(){
                           success_msg('رسالة','تمت عملية الترحيل بنجاح ..');
