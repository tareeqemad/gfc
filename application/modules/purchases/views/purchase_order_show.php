...
$get_sub_civil_type = base_url("$MODULE_NAME/$TB_NAME/public_get_sub_civil_type");
...
if ($order_purpose == 1) $DET_TB_NAME = 'public_get_details'; elseif ($order_purpose == 2) $DET_TB_NAME = 'public_get_det_items';
else
    die('show');
...
$civil_type_select1 = (count($rs) > 0) ? $rs['CIVIL_TYPE'] : -10;
$sub_civil_type = modules::run("$MODULE_NAME/$TB_NAME/public_get_sub_civil_type_p", $civil_type_select1);
$items_hidden = 'hidden';
$civil_hidden = 'hidden';
$table_rep = '';
if (count($rs) != 0) {
    if ($rs['PURCHASE_ORDER_CLASS_TYPE'] == 11) {
        $items_hidden = 'hidden';
        $civil_hidden = '';
        $table_rep = 'details_items_tb';
    } else {
        $items_hidden = '';
        $civil_hidden = 'hidden';
        $table_rep = 'details_tb';
    }
}
//die;
...
<div class="row">
    <div class="toolbar">
        ...
        <ul>
            <?php if (0 and HaveAccess($create_url)): ?>
        ...
        <form class="form-vertical" id="<?= $TB_NAME ?>_form" method="post" action="<?= $post_url ?>" role="form"
            ...
            <div class="modal-body inline_form">
                ...
                <div class="form-group col-sm-1">
                    ...
                    <div>
                        <input type="text" readonly value="<?= $HaveRs ? $rs['PURCHASE_ORDER_ID'] : '' ?>"
                        ...
                        <?php if ((isset($can_edit) ? $can_edit : false) or $can_edit_quote or $can_edit_approved) : ?>
                            <input type="hidden" value="<?= $HaveRs ? $rs['PURCHASE_ORDER_ID'] : '' ?>"
                                   name="purchase_order_id" id="h_purchase_order_id">
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="form-group col-sm-1">
                    ...
                    <div>
                        <input type="text" value="<?= $HaveRs ? $rs['PURCHASE_ORDER_NUM'] : '' ?>" readonly
                    ...
                    <div>
                        <select id="dp_adopt" disabled class="form-control">
                            ...
                            <?php foreach ($adopts as $row) : ?>
                                <option <?= $HaveRs ? ($rs['ADOPT'] == $row['CON_NO'] ? 'selected' : '') : '' ?>
                ...
                <div class="form-group col-sm-1">
                    ...
                    <div>
                        <select name="branch" id="dp_branch" <?= ($can_select_branch) ? '' : 'disabled' ?>
                                class="form-control">
                            <option value="">_________</option>
                        ...
                        <?php if ($purchase_type <= 3) { ?>
                            <select name="purchase_type" id="dp_purchase_type" disabled class="form-control">
                                ...
                                <?php foreach ($purchase_types as $row) : ?>
                                    <option <?= $HaveRs ? ($rs['PURCHASE_TYPE'] == $row['CON_NO'] ? 'selected' : '') : '' ?>
                                        value="<?= $row['CON_NO'] ?>"><?= $row['CON_NAME'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        <?php } else if ($purchase_type == 4) {
                ...
                <?php if (!$isCreate and $rs['ADOPT'] >= 60) {
                    if ($rs['PURCHASE_TYPE'] == 1) $style = ''; else
                        $style = 'display: none';
                    ?>
                        ...
                        <div>
                            <select id="dp_quote_curr_id" class="form-control">
                                <?php foreach ($currencies as $row) : ?>
                                    <option <?= $HaveRs ? ($rs['QUOTE_CURR_ID'] == $row['CURR_ID'] ? 'selected' : '') : '' ?>
                    ...
                    <div>
                        <select name="purchase_order_class_type" id="dp_purchase_order_class_type" class="form-control">
                            ...
                            <?php foreach ($purchase_order_class_types as $row) : ?>
                                ...
                                if (count($rs) == 0) {
                                    ?>
                                    <option <?= $HaveRs ? ($rs['PURCHASE_ORDER_CLASS_TYPE'] == $row['CON_NO'] ? 'selected' : '') : '' ?>
                                        value="<?= $row['CON_NO'] ?>"><?= $row['CON_NAME'] ?></option>
                                    <?php
                                } else {
                                    if ($rs['PURCHASE_ORDER_CLASS_TYPE'] == $row['CON_NO']) {
                                        ?>
                                        <option <?= $HaveRs ? ($rs['PURCHASE_ORDER_CLASS_TYPE'] == $row['CON_NO'] ? 'selected' : '') : '' ?>
                                            value="<?= $row['CON_NO'] ?>"><?= $row['CON_NAME'] ?></option>
                                        <?php
                ...
                <div class="form-group col-sm-2  civil_class hidden">
                    <label class="control-label">التعاقدات اللوجستية</label>
                    <div>
                        <select name="civil_type" id="dp_civil_type" class="form-control">
                            ...
                            <?php foreach ($civil_type as $row) : ?>
                                ...
                                if (count($rs) == 0) {
                                    ?>
                                    <option <?= $HaveRs ? ($rs['CIVIL_TYPE'] == $row['CLASS_ID'] ? 'selected' : '') : '' ?>
                                        value="<?= $row['CLASS_ID'] ?>"><?= $row['CLASS_NAME_AR'] ?></option>
                                    <?php
                                } else {
                                    if ($rs['CIVIL_TYPE'] == $row['CLASS_ID']) {
                                        ?>
                                        <option <?= $HaveRs ? ($rs['CIVIL_TYPE'] == $row['CLASS_ID'] ? 'selected' : '') : '' ?>
                                            value="<?= $row['CLASS_ID'] ?>"><?= $row['CLASS_NAME_AR'] ?></option>
                                        <?php
                ...
                <div class="form-group col-sm-2  sub_civil_class hidden">
                    <label class="control-label">تصنيف التعاقدات اللوجستية</label>
                    <div>
                        <select name="sub_civil_type" id="dp_sub_civil_type" class="form-control">
                            <?php
                            if (count($rs) == 0) {
                                ?>
                                <option value="">_________</option>
                            ...
                            ?>
                            <?php foreach ($sub_civil_type as $row) : ?>
                                <?php
                                if (count($rs) == 0) {
                                    ?>
                                    <option <?= $HaveRs ? ($rs['SUB_CIVIL_TYPE'] == $row['CLASS_ID'] ? 'selected' : '') : '' ?>
                                        value="<?= $row['CLASS_ID'] ?>"><?= $row['CLASS_NAME'] ?></option>

                                    <?php
                                } else {
                                    if ($rs['SUB_CIVIL_TYPE'] == $row['CLASS_ID']) {
                                        ?>
                                        <option <?= $HaveRs ? ($rs['SUB_CIVIL_TYPE'] == $row['CLASS_ID'] ? 'selected' : '') : '' ?>
                                            value="<?= $row['CLASS_ID'] ?>"><?= $row['CLASS_NAME'] ?></option>
                                        <?php
                        ...
                        <div>
                            <select name="committee_envelopes" id="dp_committee_envelopes" class="form-control">
                                ...
                                <?php foreach ($committee_envelopess as $row) : ?>
                                    <option <?= $HaveRs ? ($rs['COMMITTEE_ENVELOPES'] == $row['COMMITTEES_ID'] ? 'selected' : '') : '' ?>
                        ...
                        <div>
                            <select name="committee_award" id="dp_committee_award" class="form-control">
                                ...
                                <?php foreach ($committee_awards as $row) : ?>
                                    <option <?= $HaveRs ? ($rs['COMMITTEE_AWARD'] == $row['COMMITTEES_ID'] ? 'selected' : '') : '' ?>
                                        value="<?= $row['COMMITTEES_ID'] ?>"><?= $row['COMMITTEES_NAME'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div>
                        <select id="dp_committee_case" disabled class="form-control">
                            ...
                            <?php foreach ($committee_cases as $row) : ?>
                                <option <?= $HaveRs ? ($rs['COMMITTEE_CASE'] == $row['CON_NO'] ? 'selected' : '') : '' ?>
                                    value="<?= $row['CON_NO'] ?>"><?= $row['CON_NAME'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                ...
                <div class="form-group col-sm-9">
                    ...
                    <div>
                        <input type="text" value="<?= $HaveRs ? $rs['NOTES'] : '' ?>" name="notes" id="txt_notes"
                               class="form-control"/>
                    </div>
                </div>
                ...
                <?php if ($HaveRs and $quote == 1) { ?>
                    <fieldset>
                        ...
                        <div class="form-group col-sm-2">
                            ...
                            <div>
                                <input type="text" name="quote_start_date"
                                       value="<?= $HaveRs ? $rs['QUOTE_START_DATE'] : '' ?>" data-type="date"
                                       data-date-format="DD/MM/YYYY" data-val="true" id="txt_quote_start_date"
                                       class="form-control" data-val-regex="التاريخ غير صحيح!"
                                       data-val-regex-pattern="<?= date_format_exp() ?>"/>
                                <span class="field-validation-valid" data-valmsg-for="quote_start_date"
                        ...
                        <div class="form-group col-sm-2">
                            ...
                            <div>
                                <input type="text" name="quote_end_date"
                                       value="<?= $HaveRs ? $rs['QUOTE_END_DATE'] : '' ?>" data-type="date"
                                       data-date-format="DD/MM/YYYY" data-val="true" id="txt_quote_end_date"
                                       class="form-control" data-val-regex="التاريخ غير صحيح!"
                                       data-val-regex-pattern="<?= date_format_exp() ?>"/>
                                <span class="field-validation-valid" data-valmsg-for="quote_end_date"
                                      data-valmsg-replace="true"></span>
                            </div>
                        </div>
                        ...
                        <div class="form-group col-sm-1">
                            <a style="width: 120px" href="javascript:" onclick="show_conditions();"
                               class="icon-btn">
                        ...
                        <div class="form-group col-sm-9">
                            <div>
                                <textarea rows="3" name="quote_condition" id="txt_quote_condition"
                                          class="form-control"><?= $HaveRs ? $rs['QUOTE_CONDITION'] : '' ?></textarea>
                            </div>
                        </div>
                            ...
                            <div class="modal-dialog" style="width: 900px">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" onclick="add_conditions();"
                                                class="btn btn-primary" data-dismiss="modal">تم
                                        </button>
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                            ...
                                            <div>
                                                ...
                                                <select name="adv_no" id="dp_adv_no" class="form-control valid">
                                                    ...
                                                    <?php foreach ($advs as $row) : ?>
                                                        <option <?= $HaveRs ? ($rs['ADVER'] == $row['ADVER_NO'] ? 'selected' : '') : '' ?>
                                                            value="<?= $row['ADVER_NO'] ?>"><?= $row['TITLE'] ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="form-group col-sm-4">
                                            ...
                                            <input name="serial" class="form-control" id="txt_serial"
                                                   value="<?= $HaveRs ? $rs['SERIAL'] : '' ?>">
                                            <div>
                                    ...
                                    <div class="modal-footer">
                                        <?PHP if (HaveAccess($adv_url)) { ?>
                                            <button type="button" onclick="add_adv();"
                                                    class="btn btn-primary" data-dismiss="modal">حفظ
                                            </button>
                                        <?php } ?>
                                        <button type="button" class="btn btn-default" data-dismiss="modal">إغلاق
                                        </button>
                ...
                <?php if ($HaveRs) { ?>
                    ...
                    <div style="clear: both;">
                        <a style="width: 100px; margin-left: 10px" href="javascript:"
                           onclick="show_adopts();" class="icon-btn">
                            <i class="glyphicon glyphicon-list"></i>
                        ...
                        ?>
                        <a style="width: 100px; margin-left: 10px" href="javascript:" onclick="show_adv();"
                           class="icon-btn">
                            <i class="glyphicon glyphicon-certificate"></i>
                            <div> إعلان خارجي</div>
                        </a>
                        <span
                            id="quote"><?php echo modules::run('attachments/attachment/index', $rs['PURCHASE_ORDER_ID'], 'quote'); ?></span>
                        <span
                            id="purchase_book"><?php echo modules::run('attachments/attachment/index', $rs['PURCHASE_ORDER_ID'], 'purchase_book'); ?></span>
                ...
                <div id="content_items" class="<?= $civil_hidden ?>">
                    <?php
                    //echo modules::run("$MODULE_NAME/$TB_NAME/public_get_det_items", ($HaveRs)?$rs['PURCHASE_ORDER_ID']:0, ($HaveRs)?$rs['ADOPT']:0, ($HaveRs)?$quote:0,($HaveRs)?$rs['PURCHASE_ORDER_CLASS_TYPE']:0 );
                    echo modules::run("$MODULE_NAME/$TB_NAME/public_get_det_display_items", ($HaveRs) ? $rs['SUB_CIVIL_TYPE'] : '', ($HaveRs) ? $rs['PURCHASE_ORDER_ID'] : 0);
            ...
            <div class="modal-footer">
                <?php
                if (HaveAccess($div_order_url) and ($purchase_type != 0) and ($adoptt == 70) and ($rs['DESIGN_QUOTE_CASE'] == 1) and ($rs['ORDER_PURPOSE'] == 1) and ($rs['DESIGN_QUOTE_USER'] == '') and ($rs['COMMITTEE_CASE'] == 1)) : //if ( $can_adopt_reversion ) :
                    ?>
                    <button type="button" onclick='divide_order();' class="btn btn-danger">تقسيم طلب الشراء
                    </button>
                <?php endif; ?>
                ...
                <?php if (!$isCreate and $adoptt != 0) : ?>
                    <?php if ($quote == 1) : ?>
                        ...
                        if ($order_purpose == 1) {
                            ...
                            <button type="button" id="print_rep" onclick="print_rep();" class="btn btn-success">
                                <i class="glyphicon glyphicon-print"></i> طباعة
                            </button>
                        <?php } else if ($order_purpose == 2) {

                            ?>
                            <button type="button" id="print_land_offer_items" class="btn btn-success">
                                <i class="glyphicon glyphicon-print"></i> طباعة
                            </button>
                            <?php
                        }
                    ...
                    <?php if ($quote != 1) : ?>
                        ...
                        if ($order_purpose == 1) {
                            ...
                            <button type="button" id="print_rep_portrait" onclick="print_rep_portrait();"
                                    class="btn btn-success"><i class="glyphicon glyphicon-print"></i> طباعة عمودي
                            </button>
                            <button type="button" id="print_rep_landscape" onclick="print_rep_landscape();"
                                    class="btn btn-success"><i class="glyphicon glyphicon-print"></i> طباعة افقي
                            </button>
                        <?php } else if ($order_purpose == 2) {
                            ?>
                            <button type="button" id="print_rep_class_portrait" onclick="print_rep_portrait();"
                                    class="btn btn-success"><i class="glyphicon glyphicon-print"></i> طباعة عمودي
                            </button>
                            <button type="button" id="print_rep_class_landscape" onclick="print_rep_landscape();"
                                    class="btn btn-success"><i class="glyphicon glyphicon-print"></i> طباعة افقي
                            </button>
                            <?php
                        }
                    ...
                    <button type="button" onclick="$('#<?= $table_rep ?>').tableExport({type:'excel',escape:'false'});"
                ...
                <?php if (!$isCreate and $adoptt != 0 and $quote) : ?>
                    ...
                    if ($order_purpose == 1) {
                        ?>
                        <button type="button" id="print_rep2" onclick="print_rep2();" class="btn btn-success"><i
                                class="glyphicon glyphicon-print"></i> طباعة 2
                        </button>
                    <?php } else if ($order_purpose == 2) {

                        ?>
                        <button type="button" id="print_port_offer_items_rep" class="btn btn-success"><i
                                class="glyphicon glyphicon-print"></i> طباعة 2
                        </button>
                        <?php
                    }
                ...
                <?php
                if (0 and $can_edit_approved and ($purchase_type != 0)) : ?>
                ...
                <?php if (HaveAccess($adopt_url . '15') and !$isCreate and $rs['ADOPT'] == 10 and $rs['BRANCH'] > 0 and ($purchase_type != 0)) : ?>
                    <button type="button" id="btn_adopt_15" onclick='adopt(15);' class="btn btn-success">
                        اعتماد المواصفات و المقاييس
                    </button>
                <?php endif; ?>
                <?php if (HaveAccess($adopt_url . '20') and !$isCreate and $rs['ADOPT'] == 15 and $rs['BRANCH'] > 0 and ($purchase_type != 0)) : ?>
                    <button type="button" id="btn_adopt_20" onclick='adopt(20);' class="btn btn-success">
                        اعتماد الجهة الطالبة/المقر
                    </button>
                <?php endif; ?>

                <?php if (HaveAccess($adopt_url . '30') and !$isCreate and $rs['ADOPT'] == 20 and $rs['BRANCH'] > 0 and ($purchase_type != 0)) : ?>
                    <button type="button" id="btn_adopt_30" onclick='adopt(30);' class="btn btn-success">
                        اعتماد المخازن
                    </button>
                <?php endif; ?>

                <?php if (HaveAccess($adopt_url . '40') and !$isCreate and $rs['ADOPT'] == 30 and $rs['BRANCH'] > 0 and ($purchase_type != 0)) : ?>
                    <button type="button" id="btn_adopt_40" onclick='adopt(40);' class="btn btn-success">
                        اعتماد الموازنة
                    </button>
                <?php endif; ?>

                <?php if (HaveAccess($adopt_url . '45') and !$isCreate and $rs['ADOPT'] == 40 and $rs['BRANCH'] > 0 and ($purchase_type != 0)) : ?>
                    <button type="button" id="btn_adopt_45" onclick='adopt(45);' class="btn btn-success">
                        اعتماد مدير الادارة المالية
                    </button>
                <?php endif; ?>
                ...
                <?php if (HaveAccess($adopt_url . '50') and !$isCreate and $rs['ADOPT'] == 45 and $rs['IS_ASSISTENT_MANAGER'] == 1 and $rs['BRANCH'] > 0 and ($purchase_type != 0) and $rs['TOTAL_COST'] >= 3001) : ?>
                    <button type="button" id="btn_adopt_50" onclick='adopt(50);' class="btn btn-success">
                        اعتماد مدير عام الشركة
                    </button>
                <?php endif; ?>
                ...
                <?php if (HaveAccess($adopt_url . '60') and !$isCreate and ($rs['ADOPT'] == 50 or ($rs['ADOPT'] == 45 and $rs['IS_ASSISTENT_MANAGER'] == 1 and $rs['TOTAL_COST'] < 3001)) and $rs['BRANCH'] > 0 and ($purchase_type != 0)) : ?>
                    <button type="button" id="btn_adopt_60" onclick='adopt(60);' class="btn btn-success">
                        اعتماد اللوازم والخدمات
                    </button>
                <?php endif; ?>

                <?php if (HaveAccess($adopt_url . '70') and !$isCreate and $rs['ADOPT'] == 60 and $rs['BRANCH'] > 0 and ($purchase_type != 0)) : ?>
                    <button type="button" id="btn_purchases_adopt" onclick='adopt(70);'
                            class="btn btn-success"> اعتماد المشتريات
                    </button>
                <?php endif; ?>

                <?php if (HaveAccess($adopt_url . '0') and !$isCreate and $rs['ADOPT'] == 10 and $rs['BRANCH'] > 0 and ($purchase_type != 0) and $rs['ENTRY_USER'] == $this->user->id) : ?>
                    <button type="button" id="btn_adopt_0" onclick='adopt(0);' class="btn btn-danger"> إلغاء
                        طلب شراء
                    </button>
                <?php endif; ?>

                <?php if ($can_adopt_reversion and ($purchase_type != 0)) : ?>
                    <button type="button" onclick='adopt_reversion();' class="btn btn-danger">ارجاع</button>
                <?php endif; ?>
                ...
                <?php if (HaveAccess($quote_case_url . '2') and !$isCreate and $quote and $rs['DESIGN_QUOTE_CASE'] == 1 and $rs['PURCHASE_TYPE'] != 1 and ($purchase_type != 0)) : ?>
                    <button type="button" onclick='quote_case(2);' class="btn btn-success"> اعتماد المشتريات
                        لعرض السعر
                    </button>
                <?php endif; ?>
                ...
                <?php if (HaveAccess($quote_case_url . '3') and !$isCreate and $quote and $rs['DESIGN_QUOTE_CASE'] == 2 and $rs['PURCHASE_TYPE'] == 3 and ($purchase_type != 0)) : ?>
                    <button type="button" onclick='quote_case(3);' class="btn btn-success"> اعتماد الجهة
                        الطالبة لعرض السعر
                    </button>
                <?php endif; ?>
                ...
                <?php if (HaveAccess($quote_case_url . '4') and !$isCreate and $quote and $rs['DESIGN_QUOTE_CASE'] == 3 and $rs['PURCHASE_TYPE'] == 3 and ($purchase_type != 0)) : ?>
                    <button type="button" onclick='quote_case(4);' class="btn btn-success"> اعتماد الادارة
                        المالية لعرض السعر
                    </button>
                <?php endif; ?>
                ...
                <?php if (HaveAccess($quote_case_url . '5') and !$isCreate and $quote and $rs['DESIGN_QUOTE_CASE'] == 4 and $rs['IS_ASSISTENT_MANAGER'] == 1 and $rs['PURCHASE_TYPE'] == 3 and ($purchase_type != 0)) : ?>
                    <button type="button" onclick='quote_case(5);' class="btn btn-success"> اعتماد مدير عام
                        الشركة لعرض السعر
                    </button>
                <?php endif; ?>
                ...
                <?php if ($purchase_type <= 3 and HaveAccess($adopt_url . '600') and !$isCreate and $rs['ADOPT'] == 70 and $rs['DESIGN_QUOTE_CASE'] == 1 and ($purchase_type != 0)) : ?>
                    <button type="button" onclick='adopt(600);' class="btn btn-danger"> ارجاع للمشتريات
                    </button>
                <?php endif; ?>
 ...
 if ($('#dp_purchase_order_class_type').val()==11)
     {
         ...
         $( ".sub_civil_class" ).removeClass( "hidden" );
         var order_purpose=2;
         //$('#content_show').html('');
     }
 else 
...
if ($('#dp_purchase_order_class_type').val()==11)
...
else 
    {
        order_purpose=1;
         ...
         $( ".sub_civil_class" ).addClass( "hidden" );
         $("#dp_civil_type").select2('val','');
         $("#dp_sub_civil_type").select2('val','');
         $('#content_items').html('');
         $( "#content_items" ).addClass( "hidden" );
     ...
     $('#dp_civil_type').select2().on('change',function(){
       ...
       get_data('{$get_sub_civil_type}',{civil_type: $(this).val()},function(data){

             $('#dp_sub_civil_type').html('');
             $('#dp_sub_civil_type').append('<option></option>');
             $("#dp_sub_civil_type").select2('val','');
             $('#content_items').html('');
          
            $.each(data,function(index, item)
            {
            $('#dp_sub_civil_type').append('<option value=' + item.CLASS_ID + '>' + item.CLASS_NAME_AR + '</option>');
            });
            });

        });
 
 $('#dp_sub_civil_type').select2().on('change',function(){
        var civil_type=$( "#dp_sub_civil_type" ).val();
        var purchase_order_id=$( "#txt_purchase_order_id" ).val();
         get_data( '{$civile_data_url}', {civil_type:civil_type,purchase_order_id:purchase_order_id}, function( data ) {
    ...
    $('#dp_section_no, select[name="class_unit[]"],#dp_adv_no,#dp_civil_type,dp_sub_civil_type').select2();
...
//alert(ret);
                    ...
                    if(no==40)
                    {
                     var sub= 'اعتماد طلب شراء {$rs['PURCHASE_ORDER_ID']}';
                        ...
                        _send_mail(btn__,'hsalman@gedco.ps',sub,text);
                        btn__ = '';
                    }
                    if(no < 70){
...
*/
    ...
    $('#print_land_offer_items').click(function(){
            _showReport('{$report_url}&type=pdf&report=Offer_Price_Order_2_Landscape&p_id={$rs['PURCHASE_ORDER_ID']}&sn={$report_sn}',true);
    });
    ...
    $('#print_rep_class_portrait').click(function(){
            _showReport('{$report_url}&type=pdf&report=purchase_order_portrait_2&p_id={$rs['PURCHASE_ORDER_ID']}&sn={$report_sn}',true);
    });
    ...
    $('#print_rep_class_landscape').click(function(){
            _showReport('{$report_url}&type=pdf&report=purchase_order_landscape_2&p_id={$rs['PURCHASE_ORDER_ID']}&sn={$report_sn}',true);
    });
    ...
    $('#print_port_offer_items_rep').click(function(){
            _showReport('{$report_url}&type=pdf&report=Offer_Price_Order_2_Portrait&p_id={$rs['PURCHASE_ORDER_ID']}&sn={$report_sn}',true);
    });
