...
$updateGroupMember_url=base_url('purchases/suppliers_offers/updateGroupMember');

?>
<style>
    th a {
        color: #0f42b9 !important;
    }
</style>
<div class="tbl_container">
    <table class="table" id="receipt_class_input_groupTbl" data-container="container">
        ...
        <?php foreach($rec_groups as $row1) :?>
            ...
            <tr>
                ...
                <td>
                    <select class="form-control" name="status[<?= $count1 ?>]"   id="status_<?= $count1 ?>">
                        <option <?= $row1['STATUS']==1 ? 'selected' : ''  ?> value="1">حضور</option>
                        <option <?= $row1['STATUS']==2 ? 'selected' : ''  ?> value="2">غياب</option>
                    </select>
                </td>
    ...
    <div class="modal-footer">
        <?php if (HaveAccess($updateGroupMember_url)) {?>
            <button type="button" onclick="updateGroupMember();" class="btn btn-success">
                تحديث أعضاء اللجنة
            </button>
        <?php }?>

        <button type='button' onclick="addGroup(this);"  class='btn btn-primary'>حفظ بيانات الأعضاء </button>
    </div>
</div>
