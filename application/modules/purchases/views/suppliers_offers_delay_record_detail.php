    ...
    <table class="table" id="suppliers_offers_detTbl" data-container="container">
        ...
        <tr>
            ...
            <?php foreach ($suppliers_offers_data as $row) : ?>
                <th colspan="9" style="width: 300px"><?= $row['CUST_NAME'] ?>
                    <span style="display: block;" data-val="<?= $row['SUPPLIERS_OFFERS_ID'] ?>"
                          data-type="suppliers-totals"
                    >
                </th>
            <?php endforeach; ?>
            <th></th>
        ...
        <tr>
            ...
            <?php foreach ($suppliers_offers_data as $row) : ?>
                ...
                <th>
                    <input  data-val="<?= $row['SUPPLIERS_OFFERS_ID'] ?>" style="width: 40px" type="text" name="suppliers_discount[<?= $row['SUPPLIERS_OFFERS_ID'] ?>]"
                           min="0" value="<?= $row['SUPPLIERS_DISCOUNT'] ?>"
                           id="suppliers_discount_<?= $row['SUPPLIERS_OFFERS_ID'] ?>" data-val-required="حقل مطلوب"
                           data-val-regex="المدخل غير صحيح!" class="form-control percent-input">
        ...
        <tr>
            ...
            <?php foreach ($suppliers_offers_data as $row) : ?>
                ...
                <th></th>
                <th>
                    <input style="width: 40px" type="text" name="discount_value[<?= $row['SUPPLIERS_OFFERS_ID'] ?>]"
            ...
            <tr>
                ...
                <td>
                    <select id="dp_award_delay_decision_<?= $row1['CLASS_ID'] ?>"
                            ...
                            class="form-control col-sm-1 award_decision">
                        ...
                        <?php if ($row1['AWARD_DELAY_DECISION'] != 3) { ?>
                            ...
                            <option <?= ($row1['AWARD_DELAY_DECISION'] == 3) ? 'selected' : '' ?> value="3">تنفيذ
                            </option>
                            <option <?= ($row1['AWARD_DELAY_DECISION'] == 3) ? 'selected' : '' ?> value="0">إلغاء
                            </option>
                        <?php } else { ?>
                ...
                <td>
                    <textarea rows="2" name="award_delay_decision_hint[<?= $row1['CLASS_ID'] ?>]"
                              ...
                              style="width: 169px; height: 45px;"><?= $row1['AWARD_DELAY_DECISION_HINT'] ?></textarea>
                ...
                <td><input type="hidden" name="purchase_amount[]" value="<?= $row1['APPROVED'] ?>"
                ...
                foreach ($suppliers_offers_data as $row) : ?>
                    <td data-type="amount"
                        id="amount_<?= $row['SUPPLIERS_OFFERS_ID'] ?>_<?= $row1['CLASS_ID'] ?>"
                        style="background:<?= class_name($cust_amount[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']], $row1['APPROVED']) ?>"
                        class="amountxx"> <?= $cust_amount[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] ?></td>
                    <!--   <td  > <?= $cust_cust_price[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] ?></td>-->
                    <td class="price"> <?= $cust_price[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] ?></td>
                    ...
                    <td>
                        ...
                        <input data-type="approved-amount"
                               data-val="<?= $row['SUPPLIERS_OFFERS_ID'] ?>"
                               type="text"
                               max="<?= $cust_amount[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] ?>"
                               name="approved_amount[<?= $row['SUPPLIERS_OFFERS_ID'] ?>][<?= $row1['CLASS_ID'] ?>]"
                               data-val="true"
                               id="approved_amount_<?= $row['SUPPLIERS_OFFERS_ID'] ?>_<?= $row1['CLASS_ID'] ?>"
                               value="<?= $approved_amount[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] ?>"
                    ...
                    <td>
                        <input data-type="discount-percent"
                               ...
                               name="class_discount[<?= $row['SUPPLIERS_OFFERS_ID'] ?>][<?= $row1['CLASS_ID'] ?>]"
                               data-val="true"
                               id="class_discount_<?= $row['SUPPLIERS_OFFERS_ID'] ?>_<?= $row1['CLASS_ID'] ?>"
                               value="<?= $cust_class_discount[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] ?>"
                               class="form-control percent-input">
                    </td>
                    <td>
                        <input data-type="discount-value"
                               data-val="<?= $row['SUPPLIERS_OFFERS_ID'] ?>"
                               type="text" min="0"
                               name="c_discount_value[<?= $row['SUPPLIERS_OFFERS_ID'] ?>][<?= $row1['CLASS_ID'] ?>]"
                               data-val="true"
                               id="c_discount_value_<?= $row['SUPPLIERS_OFFERS_ID'] ?>_<?= $row1['CLASS_ID'] ?>"
                               value="<?= $cust_class_discount_value[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] ?>"
                               class="form-control">
                    </td>
                    <td><input data-type="approved-price"
                               data-val="<?= $row['SUPPLIERS_OFFERS_ID'] ?>"
                               type="text"
                               readonly
                               name="approved_price[<?= $row['SUPPLIERS_OFFERS_ID'] ?>][<?= $row1['CLASS_ID'] ?>]"
                               data-val="true"
                               id="approved_price_<?= $row['SUPPLIERS_OFFERS_ID'] ?>_<?= $row1['CLASS_ID'] ?>"
                               value="<?= $cust_approved_price[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] ?>"
                               class="form-control ">
                    </td>
                    <td data-type="approved-total"
                        data-val="<?= $row['SUPPLIERS_OFFERS_ID'] ?>"
                        class="total_<?= $row['SUPPLIERS_OFFERS_ID'] ?>">
                        <?php $aval = $cust_approved_price[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] * $cust_amount[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] - ($cust_approved_price[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] * $cust_amount[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] * $cust_class_discount[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] / 100);
                        echo $aval;
                    ...
                    <td>
                        <textarea rows="2"
                                  name="award_hints[<?= $row['SUPPLIERS_OFFERS_ID'] ?>][<?= $row1['CLASS_ID'] ?>]"
                                  id="award_hints_<?= $row['SUPPLIERS_OFFERS_ID'] ?>_<?= $row1['CLASS_ID'] ?>"
                                  class="form-control"
                                  style="width: 169px; height: 45px;"><?= $award_hints[$row['SUPPLIERS_OFFERS_ID']][$row1['CLASS_ID']] ?></textarea>
        ...
        <tr>
            ...
            <?php foreach ($suppliers_offers_data as $row) : ?>
                ...
                <th></th>
                <th></th>
                <th class="sumtotal_<?= $row['SUPPLIERS_OFFERS_ID'] ?>"
                    id="sumtotal_<?= $row['SUPPLIERS_OFFERS_ID'] ?>"><?= $asum_cust[$row['SUPPLIERS_OFFERS_ID']] ?></th>
        ...
        <tr>
            ...
            <?php foreach ($suppliers_offers_data as $row) : ?>
                ...
                <td colspan="9">
                    ...
                    <textarea name="award_notes[<?= $row['SUPPLIERS_OFFERS_ID'] ?>]"
