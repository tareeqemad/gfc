...
<div class="tbl_container">
    <table class="table" id="page_tb" data-container="container">
        ...
        <tr>
            ...
            <th>اسم المورد</th>
            <th>تاريخ الترحيل للإعتماد</th>
            <th>تاريخ اللإعتماد</th>
            <th>حالة المحضر</th>
        ...
        <?php foreach ($page_rows as $row) : ?>
            ...
            <tr ondblclick="show_row_details('<?= $row['SER_ADOPT'] ?>');" class="<?= class_name($row['ADOPT']) ?>">
                <td><?php if($row['ADOPT']>=20 and $row['SER_ADOPT']!=''){?><input type="checkbox" class="checkboxes" value="<?=$row['SER_ADOPT'] ?>" data-val="<?=$row['ORDER_ID'] ?>"  /><?php  }else  echo ''?></td>
                <td><?= $count ?></td>
                <td><?= $row['ORDER_ID'] ?></td>
                <td><?= $row['ORDER_ID_TEXT'] ?></td>
                <td><?= $row['REAL_ORDER_ID'] ?></td>
                <td><?= $row['CUSTOMER_RESOURCE_ID_NAME'] ?></td>
                <td><?= $row['SEND_DATE_ADOPT'] ?></td>
                <td><?= $row['SEND_DATE_ADOPT'] ?></td>
