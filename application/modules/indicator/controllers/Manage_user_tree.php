...
class Manage_user_tree extends MY_Controller
{
    ...
    function index($id = 0)
    {
        ...
        $data['title'] = 'منح المستخدمين صلاحية انشاء شجرة';
        $data['content'] = 'indecatior_create_menus_index';
    ...
    function get_tree()
    {
       ...
       // $resource = $this->_get_structure(0, 'getList', $user, $this->session->userdata('system_id'));
        $resource =  $this->_get_structure(0,$user);
      /*  $options = array(
            'template_head' => '<ul>',
            'template_foot' => '</ul>',
...
*/
       ...
       */
        ...
        $template = '<li><span data-id="{ID}"><input type="checkbox" name="menu_no" {IS_CHECKED} value="{ID}" />{ID_NAME}</span>{SUBS}</li>';
       // $template = '<li ><span data-id="{ID}" ondblclick="javascript:system_menu_get(\'{ID}\');"><input type="checkbox" name="menu_no" {IS_CHECKED} value="{MENU_NO}" /> {ID_NAME}</span>{SUBS}</li>';
        //$data['tree'] = '<ul class="tree" id="user_menus"><li><span>القوائم</span><ul>' . generate_list($resource, $options, $template) . '</ul></li></ul>';
        $data['tree'] = '<ul class="tree" id="indicator_tree"><li><span>شجرة القطاعات و التصنيفات الرئسية و الفرعية</span><ul>'.generate_list($resource, $options, $template).'</ul>';
    ...
    function _get_structure($parent,$user) {
        $result = $this->{$this->MODEL_NAME}->get_parent_tree_sectors($parent,$user);
        ...
        foreach($result as $key => $item)
        {
            ...
            $result[$i]['subs'] = $this->_get_structure($item['ID'],$user);
