...
$TB_NAME= 'indicate_data';
$backs_url=base_url("$MODULE_NAME/$TB_NAME"); //$action
$post_url= base_url("$MODULE_NAME/$TB_NAME/save_all_data");
$get_page_url = base_url("$MODULE_NAME/$TB_NAME/get_page_data");
$save_all_url = base_url("$MODULE_NAME/$TB_NAME/save_all_data");
...
if (type == 1) {
    $('input[name="value_branch_seq[]"]').each(function (i) {
...
danger_msg('يتوجب عليك حفظ الكشف اولا');
 ...
 if(val.length > 0){
     get_data('{$adopt}',{sector:$('#dp_sector').val(),txt_for_month:$('#txt_for_month').val(),entry_way:$('#dp_entry_way').val(),adopt:2,branch:$('#dp_br').val()},function(data){
...
danger_msg('يتوجب عليك حفظ الكشف اولا');
 ...
 if(val.length > 0){
     get_data('{$unadopt}',{sector:$('#dp_sector').val(),txt_for_month:$('#txt_for_month').val(),entry_way:$('#dp_entry_way').val(),adopt:1,branch:$('#dp_br').val()},function(data){
...
else
{
       ...
       var values= {page:1,sector:$('#dp_sector').val(),txt_for_month:$('#txt_for_month').val(),entry_way:$('#dp_entry_way').val(),branch:$('#dp_br').val()};
...
search();
  ...
  get_data('{$get_is_adopt_url}',{sector:$('#dp_sector').val(),txt_for_month:$('#txt_for_month').val(),entry_way:$('#dp_entry_way').val(),branch:$('#dp_br').val()} ,function(data){
  if(data=='')
  {
...
search();
 ...
 $(document).ready(function() {
      ...
      get_data('{$get_sector}',{sector:$('#dp_sector').val(),txt_for_month:$('#txt_for_month').val(),entry_way:$('#dp_entry_way').val(),branch:$('#dp_br').val()},function(data){
