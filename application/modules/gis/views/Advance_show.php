...
#map {
 ...
 var search = new Search({
	  ...
	  var  M_V_NETWORK   = new FeatureLayer("https://10.200.10.7:6443/arcgis/rest/services/GIS_GEDCO/s/MapServer/2", {
        mode: FeatureLayer.MODE_ONDEMAND,
        outFields: ["*"],
      ...
      var  ROOMS_NEW   = new FeatureLayer("https://10.200.10.7:6443/arcgis/rest/services/GIS_GEDCO/s/MapServer/3", {
        mode: FeatureLayer.MODE_ONDEMAND,
        outFields: ["*"],
	  ...
	  var  GAZA_STRIP_NEIGHBORHOODS  = new FeatureLayer("https://10.200.10.7:6443/arcgis/rest/services/GIS_GEDCO/s/MapServer/4", {
        mode: FeatureLayer.MODE_ONDEMAND,
        outFields: ["*"],
		
		  infoTemplate: new InfoTemplate("GAZA_STRIP_NEIGHBORHOODS", "${*}")
		 // infoTemplate: new InfoTemplate("Parcels", "Owner name: ${OBJECTID_1}</br>Parcel ID: ${DISTRICTID}</br>Site address: ${DISTRICTID}")
...
sources.push({
	  ...
	  sources.push({
        ...
        minCharacters: 0
	  	  ...
	  	  sources.push({
        featureLayer: GAZA_STRIP_NEIGHBORHOODS ,
        searchFields: ["OBJECTID_1"],
        displayField: "OBJECTID_1",
        exactMatch: false,
        name: "GAZA_STRIP_NEIGHBORHOODS",
        outFields: ["*"],
        placeholder: "GAZA_STRIP_NEIGHBORHOODS OBJECTID",
        maxResults: 6,
        maxSuggestions: 6,

        //Create an InfoTemplate

        infoTemplate: new InfoTemplate("GAZA_STRIP_NEIGHBORHOODS  information",
          "${*}"
        ),
...
switch (jsLang) { 
	...
	//alert(currentLayer.name);
		...
		currentLayer.show();
		break;
		case 'SDE.GAZA_STRIP_NEIGHBORHOODS': 
		currentLayer.show();
		break;
	default:
		alert('Nobody sucks!');
