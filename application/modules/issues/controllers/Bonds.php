 ...
 *
 */
    function to_issues_get($ser){


      $result =$this->{$this->MODEL_NAME}->to_issues_get($ser);
...
/*
    ...
    function create_edit_bonds(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $res= $this->{$this->MODEL_NAME}->edit_deliver(($this->edit_deliver()));
    ...
    function edit_deliver()
    {
        ...
        $result = array(
            ...
            array('name' => 'DELIVERS_STATUS_IN', 'value' =>$this->p_delivers_status, 'type' => '', 'length' => -1),
            array('name' => 'DELIVERS_DATE_IN', 'value' =>$this->p_delivers_date, 'type' => '', 'length' => -1),
            array('name' => 'DELIVERS_NOTES_IN', 'value' =>$this->p_delivers_notes, 'type' => '', 'length' => -1),
    ...
    function public_get_url($id=0)
    {
        ...
        $data['content'] = 'deliver_bounds_page';
        $data['details']= $this->{$this->MODEL_NAME}->get_bond($id);
        $data['id']=$id;
        $this->_look_ups($data);
    ...
    function _look_ups(&$data){
        ...
        $data['delivers_status'] = $this->constant_details_model->get_list(413);
    ...
    function get_page($page = 1,$sub_no= -1,$sub_name= -1,$id= -1,$bond_no= -1,$bond_year= -1,$adopt=-1,$branch=-1)
    {
        ...
        $where_sql .= isset($branch) && $branch != null ? " AND  BRANCH ={$branch}" : "";
        $where_sql .= isset($this->p_insert_user) && $this->p_insert_user != null ? " AND  insert_user ={$this->p_insert_user}" : "";
        $where_sql .= isset($this->p_update_user) && $this->p_update_user != null ? " AND  update_user ={$this->p_update_user}" : "";
        $where_sql .= isset($this->p_from_bond_date) && $this->p_from_bond_date != null ? " AND  bond_date >='{$this->p_from_bond_date}'" : "";
        $where_sql .= isset($this->p_to_bond_date) && $this->p_to_bond_date != null ? " AND  bond_date <='{$this->p_to_bond_date}'" : "";
        $where_sql .= isset($this->p_delivers_status) && $this->p_delivers_status != null ? " AND  delivers_status ={$this->p_delivers_status}" : "";
        $where_sql .= isset($this->p_from_delivers_date) && $this->p_from_delivers_date != null ? " AND  delivers_date >='{$this->p_from_delivers_date}'" : "";
        $where_sql .= isset($this->p_to_delivers_date) && $this->p_to_delivers_date != null ? " AND  delivers_date <='{$this->p_to_delivers_date}'" : "";
        //$where_sql .= isset($this->p_attach) && $this->p_attach != null ? " AND  HAVE_ATTACH ={$this->p_attach}" : "";
        //echo $where_sql.'<br/>';
        ...
        $data['page'] = $page;
        $tcount=$this->{$this->MODEL_NAME}->bound_count($where_sql);
        $data['numrecorde']= $tcount[0]['NUM_ROWS'];
        $this->_look_ups($data);
