        ...
        $.each(tools,function(i){
            ...
            if($('input[name="class_id[]"][value="'+tools[i].CLASS_ID+'"]').length <= 0){
               ...
               if($('input[name="class_id[]"]', $('#toolsTbl > tbody > tr:last')).val() != '')
                ...
                $('input[ name="work_order_id_name[]"]', $('#toolsTbl > tbody > tr:last')).val(tools[i].WORK_ORDER_ID);
...
function AddRowWithData(id , name ,start_date,end_date,type){
    ...
    if($('input[name="work_order_id[]"][value="'+id+'"]').length <= 0){
        ...
        $('#action_start_txt_work_order_id_'+(count -1)).val(start_date);
        $('#action_end_txt_work_order_id_'+(count -1)).val(end_date);
        count++;
