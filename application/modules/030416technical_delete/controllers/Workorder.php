...
class WorkOrder extends MY_Controller
{
    ...
    function get_page($page = 1, $action = 'get', $source_id= -1)
    {
        ...
        $sql.= ($source_id!= null)? " and source_id= '{$source_id}' " : '';
        ...
        $count_rs = $this->get_table_count(' WORK_ORDER_TB M WHERE 1=1 '.$sql);
    ...
    function public_get_page($txt = null, $page = 1, $action = 'get')
    {
        ...
        $sql .= isset($this->p_work_order_code) && $this->p_work_order_code != null ? " AND M.work_order_code like '%{$this->p_work_order_code}%' " : '';

        $sql .= " AND M.WORK_PERMIT  IS NOT NULL AND M.WORK_ORDER_CASE < 3 ";
    ...
    function public_get_workorder_data_json()
    {
        ...
        foreach ($this->p_id as $id) {
            ...
            array_push($details_2, array()); // $this->WorkOrder_model->team_list($this->p_id);
