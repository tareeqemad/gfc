...
class WorkOrderAssignment extends MY_Controller
{
    ...
    function _lookUps_data(&$data)
    {
        ...
        $data['WORK_ORDER_TYPE'] = $this->constant_details_model->get_list(94);
    ...
    function get($id, $action = 'index')
    {
        ...
        $data['TEAM_COST_ROWS'] = $this->WorkOrderAssignment_model->WORK_ORDER_JOB_COST_GET($id);
    ...
    function feedback($id = 0, $action = 'index')
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $rs = $this->WorkOrderAssignment_model->FEED_BAK_WORK_ORDER_ASSIGNMENT($this->p_work_order_assignment_id, $this->p_time_return);

            //Work Orders
            if (isset($this->p_w_ser))
                for ($i = 0; $i < count($this->p_w_ser); $i++) {

                    $this->WorkOrderAssignment_model->FEED_BAK_ORDER_ASSIG_ORDER($this->p_w_ser[$i], $this->p_action_end[$i], $this->p_w_hints[$i]);
                }
