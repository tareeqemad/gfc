...
class Technical_work_tree_model extends MY_Model{
    var $PKG_NAME= "TECHNICAL_PKG";
    var $TABLE_NAME= 'TECHNICAL_WORK_TREE_TB';

    function get($id= 0){
        $cursor = $this->db->get_cursor();
        $params =array(
            array('name'=>':TECHNICAL_WORK_ID','value'=>$id ,'type'=>SQLT_INT,'length'=>-1),
    ...
    function get_child($parent= 0){
        $cursor = $this->db->get_cursor();
        $params =array(
            array('name'=>':PARENT_ID_IN','value'=>$parent ,'type'=>SQLT_INT,'length'=>-1),
            array('name'=>':REF_CUR_OUT','value'=>$cursor,'type'=>OCI_B_CURSOR),
            array('name'=>':MSG_OUT','value'=>'MSG_OUT','type'=>SQLT_CHR,'length'=>500)
        );
    ...
    function delete($id){
        $params =array(
            array('name'=>':TECHNICAL_WORK_ID','value'=>$id,'type'=>'','length'=>-1),
            array('name'=>':MSG_OUT','value'=>'MSG_OUT','type'=>SQLT_CHR,'length'=>500)
        );
