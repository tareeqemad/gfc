...
class Payment_req extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->salary_amount  = $this->input->post('salary_amount');   // *** جديد
        $this->dues_amount    = $this->input->post('dues_amount');     // *** جديد
    ...
    function batch()
    {
        $data['title']   = 'صرف رواتب - دفعة جماعية';
        $data['content'] = 'payment_req_batch';
        $this->_lookup($data);
        $this->load->view('template/template1', $data);
    }

    /* ==================== BATCH EXECUTE (server-side) ==================== */
    function batch_execute()
    {
        $result = $this->{$this->MODEL_NAME}->batch_execute(
            $this->input->post('batch_type'),
            $this->input->post('the_month'),
            $this->input->post('req_type'),
            $this->input->post('wallet_type'),
            $this->input->post('calc_method'),
            $this->input->post('percent_val'),
            $this->input->post('fixed_amount'),
            $this->input->post('pay_type'),
            $this->input->post('emp_list'),
    ...
    function edit()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (is_numeric($res) && intval($res) >= 1) { echo 1; }
            else { $this->print_error($res); }
        }
    }
    ...
    {
        if ($isEdit && !$this->input->post('req_id'))     $this->print_error('REQ_ID مطلوب للتعديل');
        if ($this->p_emp_no == '')                         $this->print_error('يجب ادخال رقم الموظف');
        if (!$this->req_type)                              $this->print_error('يجب اختيار نوع الطلب');
        if (!$this->wallet_type)                           $this->print_error('يجب اختيار نوع المحفظة');
    }
    ...
    function _postedData($typ = null)
    {
        ...
        $result = array(
            ...
            array('name' => 'SALARY_AMOUNT',  'value' => $this->salary_amount ?: null, 'type' => '', 'length' => -1),
            array('name' => 'DUES_AMOUNT',    'value' => $this->dues_amount ?: null,   'type' => '', 'length' => -1),
            array('name' => 'NOTE',           'value' => $this->note,                  'type' => '', 'length' => -1),
        );
        if ($typ == 'create') { unset($result[0]); $result = array_values($result); }
        return $result;
    }
    ...
    function _lookup(&$data)
    {
        ...
        $data['req_type_cons']    = $this->constant_details_model->get_list(538);  // نوع الطلب
        $data['wallet_type_cons'] = $this->constant_details_model->get_list(539);  // المحفظة
        $data['calc_method_cons'] = $this->constant_details_model->get_list(540);  // طريقة الاحتساب
        $data['status_cons']      = $this->constant_details_model->get_list(541);  // الحالة
    }
}
