...
class bond extends MY_Controller
{
    ...
    function public_get_dmg_bond($id = 0,$adopt=1)

    {
        $data['details'] = $this->rmodel->get('DMG_BONDS_TB_GET', $id);
        $data['adopt']=$adopt;
        $this->_lookup($data);
        $this->load->view('bond_details', $data);
    }
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            for ($i = 0; $i < count($this->p_seq1); $i++)
            {
                if ($this->p_seq1[$i] <= 0 || $this->p_seq1[$i]=='' ) {
                    ...
                    $serDet=$this->rmodel->insert('DMG_BONDS_TB_INSERT',$this->_posteddata_details
                        (null, $this->p_bond_ser[$i], $this->p_bond_type[$i], $this->p_bond_body[$i]
                            ,$this->p_bond_date[$i],  $this->p_amount[$i],
    ...
    function _posteddata_details($ser = null, $bond_ser = null, $bond_type = null,
                                 $bond_body = null,$bond_date=null,$amount=null,
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            for ($i = 0; $i < count($this->p_seq1); $i++)
            {
                ...
                if ($this->p_seq1[$i] <= 0 || $this->p_seq1[$i]=='' ) {
                    ...
                    $x=$this->rmodel->insert('DMG_BONDS_TB_INSERT',$this->_posteddata_details
                        (null, $this->p_bond_ser[$i], $this->p_bond_type[$i], $this->p_bond_body[$i]
                            ,$this->p_bond_date[$i],  $this->p_amount[$i],
                            $this->ser,$this->user->branch,$this->user->id,'create') );
                    if (intval($x) <= 0) {
                    ...
                    $x=$this->rmodel->insert('DMG_BONDS_TB_UPDATE',$this->_posteddata_details
                        ($this->p_seq1[$i], $this->p_bond_ser[$i], $this->p_bond_type[$i], $this->p_bond_body[$i]
                            ,$this->p_bond_date[$i],  $this->p_amount[$i],
    ...
    function get_model_bond_com($id)
    {
        $result = $this->rmodel->get('DMG_MODELS_TB_GET', $id);
        $data['title'] = 'بيانات النموذج';
        $data['content'] = 'bond_comm_show';
        $data['result'] = $result;
        $data['isCreate'] = false;
        $data['hide'] = 'hidden';
        $data['action'] = 'edit';
    ...
    function public_get_dmg_bond_comm($id = 0,$adopt=1)

    {
        ...
        $data['adopt']=$adopt;
        $this->_lookup($data);
        $this->load->view('bond_comm_details', $data);
    }
