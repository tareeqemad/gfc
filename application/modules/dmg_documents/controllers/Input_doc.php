...
class input_doc extends MY_Controller
{
    ...
    function _lookup(&$data)
    {
        ...
        $data['document_copy'] = $this->constant_details_model->get_all(311);
    ...
    function public_get_dmg_input_doc($id = 0,$adopt=1)

    {
        $data['details'] = $this->rmodel->get('DMG_INPUT_DOCUMENTS_TB_GET', $id);
        $data['adopt']=$adopt;
        $this->_lookup($data);
        $this->load->view('input_doc_details', $data);
    }
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            for ($i = 0; $i < count($this->p_seq1); $i++)
            {
                if ($this->p_seq1[$i] <= 0 || $this->p_seq1[$i]=='' ) {
                    ...
                    $serDet=$this->rmodel->insert('DMG_INPUT_DOCUMENTS_TB_INSERT',$this->_posteddata_details
                        (null, $this->p_document_number[$i], $this->p_document_date[$i], $this->p_copy_document[$i]
                            ,$this->p_document_body[$i],  $this->p_amount[$i],
    ...
    function _postedData($isCreate = true)
    {
        $result = array(
            ...
            array('name' => 'MODEL_TYPE', 'value' => 6 , 'type' => '', 'length' => -1),
    ...
    function _posteddata_details($ser = null, $document_number = null, $document_date = null,
                                 $copy_document = null,$document_body=null,$amount=null,
                                 $model_no=null,$entry_branch=null,$entry_user=null,$type)
    {
        ...
        $result = array(
            ...
            array('name' => 'COPY_DOCUMENT', 'value' => $copy_document, 'type' => '', 'length' => -1),
            array('name' => 'DOCUMENT_BODY', 'value' => $document_body, 'type' => '', 'length' => -1),
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            for ($i = 0; $i < count($this->p_seq1); $i++)
            {
                ...
                if ($this->p_seq1[$i] <= 0 || $this->p_seq1[$i]=='' ) {
                    ...
                    $x=$this->rmodel->insert('DMG_INPUT_DOCUMENTS_TB_INSERT',$this->_posteddata_details
                        (null, $this->p_document_number[$i], $this->p_document_date[$i], $this->p_copy_document[$i]
                            ,$this->p_document_body[$i],  $this->p_amount[$i],
                            $this->ser,$this->user->branch,$this->user->id,'create') );
                    if (intval($x) <= 0) {
                    ...
                    $x=$this->rmodel->insert('DMG_INPUT_DOCUMENTS_TB_UPDATE',$this->_posteddata_details
                        ($this->p_seq1[$i],$this->p_document_number[$i], $this->p_document_date[$i],
                            $this->p_copy_document[$i]
                            ,$this->p_document_body[$i],  $this->p_amount[$i],
                            $this->ser,$this->user->branch,$this->user->id,'edit') );
    ...
    function get_model_input_doc_com($id)
    {
        $result = $this->rmodel->get('DMG_MODELS_TB_GET', $id);
        $data['title'] = 'بيانات النموذج';
        $data['content'] = 'input_doc_comm_show';
        $data['result'] = $result;
        $data['isCreate'] = false;
        $data['hide'] = 'hidden';
        $data['action'] = 'edit';
    ...
    function public_get_dmg_input_doc_comm($id = 0,$adopt=1)

    {
        ...
        $data['adopt']=$adopt;
        $this->_lookup($data);
        $this->load->view('input_doc_comm_details', $data);
    }
    ...
    function edit_comm_role(){
        ...
        for ($c = 0; $c < count($this->group_person_id); $c++) {
            ...
            if ($this->group_ser[$c] == 0){
                ...
                $rs= $this->receipt_class_input_group_model->edit($this->_postGroupsData($this->group_ser[$c],
                    $this->p_ser, $this->group_person_id[$c], $this->group_person_date[$c],
                    $this->emp_no[$c], $status, $this->member_note[$c], 'edit'));
            }
        }
        echo "1" ;
