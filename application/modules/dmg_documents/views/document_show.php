...
$DMG_DOC_TB='public_get_dmg_document';
                ...
                <fieldset class="field_set">
                        ...
                        <div class="col-sm-2">
                            ...
                            <select name="document_type" id="dp_document_type" class="form-control sel2">
                                ...
                                <?php foreach($document_type as $row) :?>
                                    <option  value="<?= $row['CON_NO'] ?>" <?PHP if ($row['CON_NO']==((count(@$rs)>0)?@$rs['DOCUMENT_TYPE']:0)){ echo " selected"; } ?> ><?php echo $row['CON_NAME']  ?></option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="modal-footer">
                        ...
                        <?php  if ( HaveAccess($adopt_commitees_url) && (!$isCreate and ( (count($rs)>0)? $rs['ADOPT']==2 : '' ) and  $this->user->branch == $rs['BRANCH_NO'] )) : ?>
                            <button type="button"  id="btn_unadopt" onclick="javascript:return_adopt(2);" class="btn btn-warning">تحويل الى اللجنة</button>
                        <?php  endif; ?>
