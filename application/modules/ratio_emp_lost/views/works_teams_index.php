...
<div class="page-header">
    ...
    <div class="ms-auto pageheader-btn">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">أعمال الفرق</li>
        </ol>
    </div>
</div>
<!-- PAGE-HEADER END -->
...
<!--                        <a class="btn btn-info" href="--><?//= $create_url ?><!--"><i class='glyphicon glyphicon-plus'></i>جديد </a>-->
            ...
            <div class="card-body">
                ...
                <form class="form-vertical" id="<?=$TB_NAME?>_form" >
                    <div class="row">
                        ...
                        <div class="form-group col-sm-3">
                            ...
                            <div>
                                <select type="text" name="target" id="dp_target" class="form-control sel2" >
                                    <option value="">_________</option>
                                    <?php foreach ($target as $row) : ?>
                                        <option value="<?= $row['TARGET_NO']?>"><?= $row['TARGET_NO'] . ': '.$row['BRANCH_ID_NAME'].' - '.$row['ACTIVITY_NO_NAME'].' - ' . $row['THE_MONTH'] ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                ...
                <div class="flex-shrink-0">
                    ...
                    <button type="button" onclick="$('#works_teams_tb').tableExport({type:'excel',escape:'false'});" class="btn btn-success"><i class="fas fa-file-excel"></i>إكسل</button>
...
<script type="text/javascript">
    ...
    function values_search(add_page){
        var values= {page:1,branch_no:$('#dp_branch_no').val(),the_month:$('#txt_the_month').val(),activity_no:$('#dp_activity_no').val(),target:$('#dp_target').val(),team:$('#dp_team').val()};
        if(add_page==0)
            delete values.page;
        return values;
    }
    ...
    function LoadingData(){
        ...
        ajax_pager_data('#works_teams_tb > tbody',values);
    }

    function clear_form(){
        $('#dp_target ,#dp_team ,#dp_activity_no').select2('val',0);
    }
