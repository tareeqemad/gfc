...
$get_url = base_url('payment/financial_payment/get');
...
$report_jasper_treasury_url = base_url("JsperReport/showreport?sys=financial/treasury");
$report_sn= report_sn();
?>
<?= AntiForgeryToken() ?>
<div class="row">
...
<div class="modal-footer">
    ...
    <?php if (count($rs) > 0 && $rs['FINANCIAL_PAYMENT_CASE'] >= 4 && ($rs['CHECK_ID'] != '' || $rs['PAYMENT_TYPE'] == 3) && HaveAccess($print_per_url)): ?>
       ...
       <?php if ($rs['PAYMENT_TYPE'] == 2) { ?>
            <a onclick="_showReport('<?= $report_jasper_treasury_url ?>&report=<?= $rs['PAYMENT_TYPE'] == 2 ? check_reports($rs['CHECK_ID'], $rs['CURR_ID'], $rs['CHECK_BANK_ID']) : '' ?>&p_financial_payment_id=<?= $rs['FINANCIAL_PAYMENT_ID'] ?>');"
               class="btn btn-success" href="javascript:"> طباعة الشيك</a>
...
<div class="modal fade" id="notesModal">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" id="btn_apply_action" onclick="apply_action();" class="btn btn-primary">حفظ البيانات</button>
    ...
    <div class="modal fade" id="reviewModal">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="form-group col-sm-12">
                    <label class="control-label col-sm-2">تصنيف الملاحظة </label>
                    <div class="col-sm-4">
                        <select name="note_category" id="dp_note_category" class="form-control">
                            <option value="">_____</option>
                            <?php foreach ($note_category_cons as $row) : ?>
                                <option value="<?= $row['CON_NO'] ?>"><?= $row['CON_NAME'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                ...
                <div class="form-group col-sm-12">
                    <label class="control-label col-sm-2">  ملاحظة المراقب المالي </label> <!--  old ملاحظة مدير الرقابة  -->
                    <div class="col-sm-4">
                        <select name="audit_manager_opinion" id="dp_audit_manager_opinion" class="form-control">
                            <option value="">_____</option>
                            <?php foreach ($audit_manager_opinion_cons as $row) : ?>
                                <option value="<?= $row['CON_NO'] ?>"><?= $row['CON_NAME'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                ...
                <div class="form-group col-sm-12">
                    <label class="control-label col-sm-2"> الجهة المختصة  </label>
                    <div class="col-sm-4">
                        <select name="specialized_side" id="dp_specialized_side" class="form-control">
                            <option value="">_____</option>
                            <?php foreach ($specialized_side_cons as $row) : ?>
                                <option value="<?= $row['CON_NO'] ?>"><?= $row['CON_NAME'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
...
<script>
    ...
    $('#dp_note_category').val('{$rs['NOTE_CATEGORY']}');
    $('#dp_audit_manager_opinion').val('{$rs['AUDIT_MANAGER_OPINION']}');
    $('#dp_specialized_side').val('{$rs['SPECIALIZED_SIDE']}');
    ...
    var btn__= '';
    $('#btn_apply_action').click( function(){
        btn__ = $(this);
    });
        ...
        if(action_type == 1){
            ...
            get_data(url,{financial_payment_id:{$rs['FINANCIAL_PAYMENT_ID']},hints:$('#txt_hints').val()},function(data){
                if(data =='1'){
                    ...
                    var sub= 'ارجاع سند صرف {$rs['ENTRY_SER']}';
                    var text= ' تم ارجاع سند صرف رقم {$rs['ENTRY_SER']} ';
                    text+= ' - بيان/ '+ $('#txt_hints').val();
                    text+= ' - الملاحظة/ '+ $('#txt_g_notes').val();
                    text+= ' <br> {$gfc_domain}{$get_url}/{$rs['FINANCIAL_PAYMENT_ID']}/archive';
                    _send_mail(btn__,'{$treasury_email}',sub,text);
                    btn__ = '';
                }
    ...
    function save_review_doc(){
        if($('#txt_review_doc').val().length > 0){
            if( $('#dp_note_category').val()=='' || $('#dp_audit_manager_opinion').val()=='' || $('#dp_specialized_side').val()==''  ){
                warning_msg('تنبيه','يجب ادخال/ تصنيف الملاحظة -  ملاحظة المراقب المالي -  الجهة المختصة ');
                return 0;
            }
            
               get_data('{$review_doc}',{id:{$rs['FINANCIAL_PAYMENT_ID']},notes:$('#txt_review_doc').val(),review_type: $('#dp_review_type').val(), note_category: $('#dp_note_category').val(), audit_manager_opinion: $('#dp_audit_manager_opinion').val(), specialized_side: $('#dp_specialized_side').val()},function(data){
