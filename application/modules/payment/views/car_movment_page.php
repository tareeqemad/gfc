...
$changeStatus = base_url("$MODULE_NAME/$TB_NAME/changeStatus");
$count = 1;
?>
    ...
    <table class="table" id="chainsTbl" data-container="container">
        ...
        <?php foreach ($rows as $row) : ?>
            <tr>
                ...
                <td><?= $row['OFFICE_ID_NAME'] ?></td>
                <td><?= $row['CAR_ID'] . " - " . $row['CAR_ID_NAME'] ?></td>
                <td><?= $row['DRIVER_ID']?></td>
                <td><?= $row['DRIVER_ID_NAME']?></td>
                <td><?= $row['MOVMENT_TYPE_NAME'] ?></td>
                <td><?= $row['DATE_MOVE'] ?></td>
                <td><?= $row['ENTRY_DATE'] ?></td>
                <td><?= $row['ENTRY_USER_NAME'] ?></td>

                <?php if ( $row['DETAILS_COUNT'] >= 1 ) :  ?>
                    <td><span class="badge badge-2">مسندة</span></td>
                <?php else : ?>
                ...
                <td>
                    <a href="<?= base_url("payment/carMovements/get/{$row['SER']}") ?>" class="btn btn-info btn-xs">إضافة</a>

                    <?php if ( HaveAccess($changeStatus) ) :  ?>
