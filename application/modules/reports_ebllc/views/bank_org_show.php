                    ...
                    <form class="form-vertical" id="<?=$TB_NAME?>_form" method="post" role="form" action="<?=$post_url?>" novalidate="novalidate">
                        <div class="row">
                            ...
                            <div class="form-group col-sm-2">
                                ...
                                <select type="text" name="bank_type" id="dp_bank_type" class="form-control sel2"  onchange="get_bank_type();">
                                    <option value="">__________</option>
                                    <?php foreach ($bank_type as $row) : ?>
                                        <option  <?=$HaveRs?($rs['BANK_TYPE']==$row['B_TYPE']?'selected':''):''?>  value="<?=$row['B_TYPE']?>"><?=$row['BANK_TYPE_NAME']?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="form-group col-sm-1">
                                ...
                                <input type="text" value="<?=$HaveRs?$rs['BANK_NO']:''?>" name="bank_no" id="txt_bank_no" class="form-control"  onkeypress="return (event.charCode !=8 && event.charCode ==0 || ( event.charCode == 46 || (event.charCode >= 48 && event.charCode <= 57)))"/>
                            </div>

                            <div class="form-group col-sm-2">
                                <label>اسم البنك</label>
                                <div>
                                    <input type="text" value="<?=$HaveRs?$rs['BANK_NAME']:''?>" name="bank_name" id="txt_bank_name" class="form-control"/>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    function get_bank_type(){
        var no =  $('#dp_bank_type').val();
        if (no == 0) {
            $('#txt_bank_no').val('');   
            return -1; 
        } else {
