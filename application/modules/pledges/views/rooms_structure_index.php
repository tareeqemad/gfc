...
$all_branches_url= base_url("$MODULE_NAME/$TB_NAME/all_branches");
$all_branches= HaveAccess($all_branches_url)?1:0;
...
<div class="row">
    <div class="toolbar">
        ...
        <ul>
            ...
            <li><a  onclick="javascript:print_room_barcode();" href="javascript:;"><i class="glyphicon glyphicon-barcode"></i>طباعة</a> </li>
...
<script>
    ...
    function get_emps(id){
        ...
        if( id.substr(0,1) == '{$user_branch}' || '{$all_branches}'==1){
    ...
    function print_room_barcode(){
        var chk_rooms= $('#rooms_structure_tree .checkboxes:checked');
        var rooms_no= '';

        chk_rooms.each(function(i){
            rooms_no += $(this).closest('span').attr('data-no') + '_:_';
        }); // each
        _showReport('{$barcode_tlp1_url}/'+ (rooms_no).slice(0, -3) );
    }
