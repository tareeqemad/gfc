...
class inventory_pledges extends MY_Controller
{
    ...
    function index($page= 1, $emp_pledges=-1, $file_id= -1, $yyear= -1, $customer_id= -1, $source= -1, $status= -1, $class_id= -1, $class_unit= -1, $exp_account_cust= -1, $class_output_id= -1, $notes= -1, $class_code_ser= -1,$class_type=-1,$serial=-1,$is_adopt=-1,$is_added=-1,$is_exist=-1,$branch=-1,$tdate=-1,$fdate=-1,$adopt=-1,$personal_custody_type=-1,$custody_type=-1,$room_id=-1,$customer_type=-1){ //,$manage_st_id=-1,$cycle_st_id=-1,$department_st_id=-1
		$this->last_year=$this->{$this->MODEL_NAME}->get_inventory_year()[0]['LAST_YEAR'];
		
        if ($emp_pledges==1)
            $data['title']='جرد عهد السنوات السابقة ';
        else
            $data['title']=' جرد العهد لسنة' .$this->last_year;
...
//print_R($data['branches']);
     ...
     //   $data['department_st_id']=$department_st_id;
        ...
        if ($emp_pledges==2 and $this->last_year==0)
            die('الجرد غير مفتوح');
    ...
    function get_page($page= 1, $emp_pledges= -1, $file_id= -1, $yyear= -1, $customer_id= -1, $source= -1, $status= -1, $class_id= -1, $class_unit= -1, $exp_account_cust= -1, $class_output_id= -1, $notes= -1,$class_code_ser=-1,$class_type=-1,$serial=-1,$is_adopt=-1,$is_added=-1,$is_exist=-1,$branch=-1,$tdate=-1,$fdate=-1,$adopt=-1,$personal_custody_type=-1,$custody_type=-1,$room_id=-1,$customer_type=-1){ //,$manage_st_id=-1,$cycle_st_id=-1,$department_st_id=-1

        $this->load->library('pagination');
	      $this->load->model('settings/constant_details_model');
			 $data['pledge_reasons'] = $this->constant_details_model->get_list(455);
	
        $file_id= $this->check_vars($file_id,'file_id');
		$yyear= $this->check_vars($yyear,'yyear');
      ...
      //  $customer_id2= $this->check_vars(-1,'customer_id2');
        ...
        $customer_type= $this->check_vars($customer_type,'customer_type');
     ...
     //   $department_st_id= $this->check_vars($department_st_id,'department_st_id');
       ...
       $this->last_year=$this->{$this->MODEL_NAME}->get_inventory_year()[0]['LAST_YEAR'];
       $data['last_year']=$this->last_year ;
	   if ($data['last_year']==0) $data['last_year']=date('Y');
	   $data['emp_pledges']=$emp_pledges ;
        if ($emp_pledges==2 and !HaveAccess('pledges/inventory_pledges/index/1/2'))
            die('NotHaveAccess');
        ...
        $where_sql.= ($yyear!= null)? " and yyear= '{$yyear}' " : ''; 
	
        $where_sql.= ($file_id!= null)? " and file_id= '{$file_id}' " : '';
        $where_sql.= ($emp_pledges!= null && $emp_pledges==1)? " and yyear <= ".$data['last_year'] : '';
     //   $where_sql.= ($customer_id!= null)? " and customer_id= '{$customer_id}' " : '';
        //$where_sql.= ($customer_id2!= null and $customer_type==4)? " and customer_id= '{$customer_id2}' " : '';
		  ...
		  $where_sql.= ($customer_id!= null and $customer_type!=4)? " and customer_id= '{$customer_id}' " : '';
        $where_sql.= ($room_id!= null and $customer_type==4)? " and customer_id= '{$room_id}' " : '';
...
/*
    function adopt()
	    {
			$this->last_year=$this->{$this->MODEL_NAME}->get_inventory_year()[0]['LAST_YEAR'];
	if($this->last_year ==0) die('لا يمكن إضافة عهدة .. تم إغلاق الجرد');
    ...
    {
        $create_time= time();
        if($create_time < $this->session->userdata('ip_adopt_moves_create_time') + 20){
            $this->print_error("انتظر 20 ثانية ثم حاول مرة اخرى"); die();
        }
        $this->session->set_userdata('ip_adopt_moves_create_time', $create_time);
	...
	if($this->last_year ==0) die('لا يمكن إضافة عهدة .. تم إغلاق الجرد');
        ...
        if (is_array($id)) {
            foreach ($id as $val) {
                $msg = $this->{$this->MODEL_NAME}->customers_insert($val);
            }
        } else {
            $msg = $this->{$this->MODEL_NAME}->customers_insert($id);
        }

        if ($msg >= 1) {
            //echo modules::run($this->PAGE_URL);
        } else {
          die($msg );  $this->print_error_msg($msg);
        }
    }
	...
	if($this->last_year ==0) die('لا يمكن إضافة عهدة .. تم إغلاق الجرد');
	    ...
	    function update_reasons(){ 
			$this->last_year=$this->{$this->MODEL_NAME}->get_inventory_year()[0]['LAST_YEAR'];
	if($this->last_year ==0) die('لا يمكن إضافة عهدة .. تم إغلاق الجرد');
        if ($_SERVER['REQUEST_METHOD'] == 'POST' and $this->p_id!='' and $this->p_pledge_reason!=''  ){
          //  $res = $this->{$this->MODEL_NAME}->is_exist($this->p_id,$this->p_pledge_reason,$this->p_pledge_reson_notes);
		  $res = $this->{$this->MODEL_NAME}->update_reasons($this->p_id,$this->p_pledge_reason,$this->p_pledge_reson_notes);
            ...
            echo "لم يتم إرسال رقم العهدة و اختيار السبب";
    }
    function exist_pledge_barcode(){
			$this->last_year=$this->{$this->MODEL_NAME}->get_inventory_year()[0]['LAST_YEAR'];
	if($this->last_year ==0) die('لا يمكن إضافة عهدة .. تم إغلاق الجرد');
        if ($_SERVER['REQUEST_METHOD'] == 'POST' and $this->p_customer_type!='' and $this->p_customer_id!='' and $this->p_barcode!=''){
            $res = $this->{$this->MODEL_NAME}->is_exist_barcode($this->p_customer_type,$this->p_customer_id,$this->p_barcode);

            if(intval($res) <= 0){
				$this->print_error('الباركود غير موجود '.'<br>'.$res);
            }
            echo 1;
        }else
            echo "لم يتم ارسال رقم العهدة و الحالة";
 ...
 */
    ...
    function movement($id=0){
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $customer_movment_id_=($this->customer_movment_type==4)?$this->customer_movment_id2:$this->customer_movment_id;
    ...
    function excute(){
        $create_time= time();
        if($create_time < $this->session->userdata('ip_excute_create_time') + 25){
            $this->print_error("انتظر 25 ثانية ثم حاول مرة اخرى"); die();
        }
        $this->session->set_userdata('ip_excute_create_time', $create_time);
    ...
    function _post_validation($isEdit = false){
        ...
        if( ($this->file_id=='' and $isEdit) or ($customer_id_=='' or $this->recieved_date=='' or $this->class_id==''  or $this->class_type=='')  ){
...
*/
    ...
    function public_get_rooms($data_type=''){
	   ...
	   function  close_year(){
			 $this->last_year=$this->{$this->MODEL_NAME}->get_inventory_year()[0]['LAST_YEAR'];
	if($this->last_year ==0) {
		$this->print_error('الجرد مغلق سابقا'); die;
		}
	 else {
		     $res = $this->{$this->MODEL_NAME}->close_year();
		   if(intval($res) <= 0){
                $this->print_error('لم يتم إغلاق الجرد'.'<br>'.$res);
            } else echo 'تمت العملية بنجاح';
	 }
		   
		   

      //  echo $this->{$this->MODEL_NAME}->close_year();
    }
	
	   function  open_new_year(){
		    $this->last_year=$this->{$this->MODEL_NAME}->get_inventory_year()[0]['LAST_YEAR'];
	if($this->last_year >0) {
		$this->print_error('يجب إغلاق الجرد السابق أولا..'); die;
		}
	 else {	
		     $res = $this->{$this->MODEL_NAME}->open_new_year();//echo 1;  die;
		 /*  if(intval($res) <= 0){
                $this->print_error('لم يتم فتح جرد جديد'.'<br>'.$res);
            } else echo 'تمت العملية بنجاح';*/
    ...
    function inventory_control_index(){


        add_js('circle-progress.js');

        $data['title']='إغلاق الجرد و فتح جرد جديد';
