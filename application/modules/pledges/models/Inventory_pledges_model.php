...
class inventory_pledges_model extends MY_Model{
    var $PKG_NAME= "INVENTORY_PLEDGES_PKG";
    var $TABLE_NAME= 'INVENTORY_PLEDGES';
    ...
    function get_inventory_year(){
        
        $params =array(
		  array('name'=>':REF_CUR_OUT','value'=>'cursor','type'=>OCI_B_CURSOR),
        array('name'=>':MSG_OUT','value'=>'MSG_OUT','type'=>SQLT_CHR,'length'=>500)
        );
        $result = $this->New_rmodel->general_get($this->PKG_NAME,'INVENTORY_LYEAR_GET',$params);
        return $result;
    }
    function get($id= 0){
    ...
    function is_exist($id,$case){

        $params =array(
            array('name'=>':FILE_ID_IN','value'=>$id,'type'=>'','length'=>-1),
            array('name'=>':IS_EXIST_IN','value'=>$case,'type'=>'','length'=>-1),
            array('name'=>':MSG_OUT','value'=>'MSG_OUT','type'=>SQLT_CHR,'length'=>-1),
        );

        $result = $this->conn->excuteProcedures($this->PKG_NAME, $this->TABLE_NAME.'_IS_EXIST',$params);;

        return $result['MSG_OUT'];

    }
	
	   function update_reasons($id,$pledge_reason,$pledge_reson_notes){  //pledge_reason:pledge_reason,pledge_reson_notes:pledge_reson_notes

        $params =array(
            array('name'=>':FILE_ID_IN','value'=>$id,'type'=>'','length'=>-1),
            array('name'=>':PLEDGE_REASON','value'=>$pledge_reason,'type'=>'','length'=>-1),
			array('name'=>':PLEDGE_RESON_NOTES','value'=>$pledge_reson_notes,'type'=>'','length'=>-1),
            array('name'=>':MSG_OUT','value'=>'MSG_OUT','type'=>SQLT_CHR,'length'=>-1),
        );

        $result = $this->conn->excuteProcedures($this->PKG_NAME, $this->TABLE_NAME.'_UP_REASONS',$params); 

        return $result['MSG_OUT'];

    }

    function is_exist_barcode($customer_type,$customer_id,$barcode){
        ...
        $result = $this->conn->excuteProcedures($this->PKG_NAME, $this->TABLE_NAME.'_IS_EXIST_BAR',$params);;
    ...
    function move($id,$customer_movment_id,$class_type,$customer_movment_type,$room_movement_id){
    ...
    function customers_insert($id){
        ...
        $result = $this->conn->excuteProcedures($this->PKG_NAME, $this->TABLE_NAME.'_INSERT_CUST',$params);;
        return $result['MSG_OUT'];
    ...
    function inventory_pledges_get_file_ids($customer_type,$customer_id,$file_id){
        ...
        $result = $this->New_rmodel->general_get($this->PKG_NAME,'INVENTORY_PLEDGES_GET_FILE_IDS',$params);
        return $result;
    }
    ...
    function close_year(){
        ...
        $result = $this->conn->excuteProcedures($this->PKG_NAME, 'CLOSE_INVENTOY_YEAR',$params);;
	   ...
	   function open_new_year(){
        ...
        $result = $this->conn->excuteProcedures($this->PKG_NAME,'OPEN_INVENTOY_YEAR', $params);
        return $result['MSG_OUT'];
    }
