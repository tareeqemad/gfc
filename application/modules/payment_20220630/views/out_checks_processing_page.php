...
<table class="table" id="voucherTbl" data-container="container">
    ...
    <?php foreach($checks as $check) :?>
        <tr ondblclick="javascript:get_to_link('<?= base_url('payment/out_checks_processing/get/').'/'.$check['CHECKS_PROCESSING_PAYMENT_ID'] ?>');"  class="case_<?= $check['CHECKS_PROCESSING_CASE'] ?>">

        <td><?= $count ?></td>
            <td><?= $check['CHECKS_PAYMENT_TYPE_NAME'] ?></td>
            <td><?= $check['CHECKS_PROCESSING_PAYMENT_ID'] ?></td>
            <td><?= $check['CHECKS_PROCESSING_PAYMENT_DATE'] ?></td>
