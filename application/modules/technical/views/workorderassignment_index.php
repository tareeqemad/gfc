...
$get_page_url = base_url('technical/WorkOrderAssignment/get_page');
...
<div class="row">
    ...
    <div class="form-body">
        ...
        <div id="container">
            <?php echo modules::run('technical/WorkOrderAssignment/get_page',$page,$case,$action); ?>
        </div>
...
<script>
    ...
    ajax_pager({action :'$action',work_order_type:$('#dp_work_order_type').val(),adapter_serial:$('#h_adapter_serial').val(), x:$('#txt_x').val() , y:$('#txt_y').val() ,branch : $('#dp_branch').val(), title:$('#txt_title').val() , team_id: $('#txt_team_id').val()  ,work_assignment_code : $('#txt_work_assignment_code').val()});

    }

    function LoadingData(){

    ajax_pager_data('#projectTbl > tbody',{action :'$action',work_order_type:$('#dp_work_order_type').val(),adapter_serial:$('#h_adapter_serial').val(), x:$('#txt_x').val() , y:$('#txt_y').val(),branch : $('#dp_branch').val() ,title:$('#txt_title').val() , team_id: $('#txt_team_id').val()  ,work_assignment_code : $('#txt_work_assignment_code').val()});
...
_showReport('$public_select_adapter_url/adapter_serial');
   ...
   function do_search(){
        ...
        get_data('{$get_page_url}',{page: 1,action :'$action',work_order_type:$('#dp_work_order_type').val(),adapter_serial:$('#h_adapter_serial').val(), x:$('#txt_x').val() , y:$('#txt_y').val() ,branch : $('#dp_branch').val(),title:$('#txt_title').val() , team_id: $('#txt_team_id').val()  ,work_assignment_code : $('#txt_work_assignment_code').val()},function(data){
            $('#container').html(data);
