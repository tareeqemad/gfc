        ...
        <fieldset>
            ...
            <div class="modal-body inline_form">
                ...
                <div class="form-group col-sm-1">
                    ...
                    <div>
                        <input type="text" name="work_code" id="txt_work_code"
                               class="form-control">
                    </div>
                </div>
...
<script>
    ...
    ajax_pager({action :'$action' ,work_code:$('#txt_work_code').val(),branch : $('#dp_branch').val() ,work_order_type:$('#dp_work_order_type').val() , workorder_title:$('#txt_workorder_title').val() ,from_date:$('#txt_from_date').val(),to_date:$('#txt_to_date').val(), source_id:$('#h_txt_source_id').val() , task : $('#txt_task').val() });

    }

    function LoadingData(){

    ajax_pager_data('#projectTbl > tbody',{action :'$action',work_code:$('#txt_work_code').val(),branch : $('#dp_branch').val()  ,work_order_type:$('#dp_work_order_type').val() , workorder_title:$('#txt_workorder_title').val() ,from_date:$('#txt_from_date').val(),to_date:$('#txt_to_date').val() ,source_id:$('#h_txt_source_id').val() , task : $('#txt_task').val()});
   ...
   function do_search(){
        ...
        get_data('{$get_page_url}',{page: 1,action :'$action' ,work_code:$('#txt_work_code').val(),branch : $('#dp_branch').val() ,work_order_type:$('#dp_work_order_type').val() , workorder_title:$('#txt_workorder_title').val() ,from_date:$('#txt_from_date').val(),to_date:$('#txt_to_date').val() ,source_id:$('#h_txt_source_id').val(), task : $('#txt_task').val() },function(data){
            $('#container').html(data);
