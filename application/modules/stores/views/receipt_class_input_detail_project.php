    ...
    <table class="table" id="receipt_class_input_detailTbl" data-container="container">
        ...
        <?php foreach($rec_details as $row) :?>
            ...
            <tr>
                ...
                <td>
                 <input type="hidden" name="unit_class_id[]" value="<?=$row['CLASS_UNIT']?>">
                    <?=$row['UNIT_NAME']?>
                  <!--  <select name="unit_class_id[]" class="form-control  col-sm-10" id="unit_class_id_<?=$count?>"  >
                        <option></option>
                        <?php // foreach($class_units as $rowx) :
                                             ?>
                            <option value="<? //=$rowx['CON_NO']?>" <?php //if($rowx['CON_NO']==$row['CLASS_UNIT']) echo "selected";?>><? //=$rowx['CON_NAME']?> </option>
                        <?php // endforeach;?>
                    </select> -->
                </td>
                <td id="class_type_class_id_<?=$count?>"><?= $row['CLASS_TYPE_NAME'] ?></td>
                <td><input name="amount[]"  data-val="true"  id="amount_<?= $count ?>"    data-val-required="حقل مطلوب"   data-val-regex="المدخل غير صحيح!"   class="form-control" value="<?= $row['AMOUNT'] ?>"  max="<?= $row['AMOUNT'] ?>"> <!--ondblclick="alert('<? //= $row['AMOUNT'] ?>'); return false ;"-->
