...
<table class="table" id="receipt_class_input_tb" data-container="container">
    ...
    <?php  foreach($get_all as $row) {  $count++; ?>
        <tr ondblclick="javascript:select_class('<?=$row['RECEORD_ID_TEXT']?>','<?=$row['ORDER_ID']?>','<?=$row['RECORD_ID']?>','<?=$row['CUSTOMER_RESOURCE_ID']?>'
            ,'<?=$row['CUST_NAME']?>','<?=$row['CLASS_INPUT_CLASS_TYPE']?>','<?=$row['STORE_ID']?>');"  class="case_<?= $row['RECEIPT_CLASS_INPUT_ID'] ?>">
