...
//var_dump($gcc_structure_show_dep);
//die;
?>
...
<?php foreach($gcc_structure_show_dep as $row) :?>
    <option  value="<?= $row['ST_ID'] ?>" <?PHP  if (count($rs)>0) {if ($row['ST_ID']==$rs['MOVMENT_DEPART_ST_ID']){ echo " selected"; }} ?> ><?php echo $row['ST_NAME']  ?></option>
<?php endforeach; ?>
</select>