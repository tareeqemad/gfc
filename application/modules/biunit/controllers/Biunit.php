...
class Biunit extends MY_Controller{
    ...
    function report_form(){
        $data['Main_categories'] = $this->Report_model->get_main_categories();
        $data['Sub_categories'] = $this->Report_model->get_sub_categories();
        $manager = $this->Report_model->get_branch_managers(get_curr_user()->username,$_GET['month']);
       // $month = date('Ym', strtotime(date('Ym')." -1 month"));

        if(empty($manager[0])){
            redirect('/biunit/Biunit');
        }
        else{
            //print_r($manager);die();
            $data['branch_id'] = $manager[0]['BRANCH_ID'];
            $data['msg'] = $manager[0]['FLAG'];
