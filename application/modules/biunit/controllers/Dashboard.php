...
class Dashboard extends MY_Controller
{
    ...
    function create(){
        ...
        redirect('Biunit/da3em_setting'.($msg==0?'?msg=حدث خطأ..لم يتم إضافة اللوحة.':''));
    }
    function update(){
        $data['ID'] = $_POST['id'];
        $data['DASHBOARD_TITLE'] = $_POST['title'];
        ...
        redirect('Biunit/da3em_setting'.($msg==0?'?msg=حدث خطأ..لم يتم تعديل اللوحة.':''));
    }
    function delete(){
        $msg = $this->Dashboard_model->delete_dashboard($_GET['id']);

        redirect('Biunit/da3em_setting'.($msg==0?'?msg=حدث خطأ..لم يتم حذف اللوحة.':''));
    }
