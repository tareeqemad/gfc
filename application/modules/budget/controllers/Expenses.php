...
class Expenses extends MY_Controller{
    ...
    var $PAGE_URL= 'budget/expenses/get_page';
    var $exp_rev_type= 2; // 2 exp - 1 rev
    ...
    function index(){
        $data['title']='النفقات';
        if ($this->year >= 2017)
            $data['content']='expenses_index2017';
        else
            $data['content']='expenses_index';

        $data['select_position']= $this->select_position();
        $data['select_section']= $this->select_section();
        $data['year']= $this->year;
    ...
    function get_page(){
     ...
     /*   if($item_no!= null and $department_no!= null){
            ...
            $this->load->view('expenses_page',$data);
        }else
        ...
        if ($yyear >= 2017){
            ...
            $this->load->view('expenses_page2017',$data);
        } else {
            if($item_no!= null and $department_no!= null){
                $data['item_data']=$this->budget_items_model->get($item_no);
                $data['get_list']= $this->{$this->MODEL_NAME}->get_list($item_no,$mmonth,$department_no,$yyear,$branch,$this->exp_rev_type);
    ...
    function receive_data(){
        ...
        if ($yyear<2017) {
            ...
            $item_no= $this->input->post('items');
  //      $a_ceil_val= $this->budget_financial_ceil_model->active_ceil_value($section_no,$branch,$yyear,2) ;
    //    if ($a_ceil_val>=$total){
    ...
    function select_items(){
        $all= $this->budget_items_model->get_list($this->input->post('section'), 0, 2, 2); // not has_history , not special
        ...
        $select.= "</select>";
        echo $select;
        $item_url= base_url("budget/expenses/item_data");
        echo "
    ...
    function attachment_get(){
        ...
        if($item!='' and $item >0 and $user_position!='' and $user_position >0 ){
            $attachment_name='exp_'.$this->user->branch.'_'.$user_position.'_'.$item.'_'.time();
            $attachment_identity='exp_'.$this->user->branch.'_'.$user_position.'_'.($this->year).'_'.$item;
