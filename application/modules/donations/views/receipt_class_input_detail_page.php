    ...
    <table class="table" id="receipt_class_input_detailTbl" data-container="container">
        ...
        <?php foreach($rec_details as $row) :?>
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="h_class_id[]"  readonly   id="h_class_id_<?= $count ?>"   value="<?= $row['CALSS_ID'] ?>"  class="form-control col-sm-2">
                </td>
                <td>
                ...
                <td><input name="amount[]"  data-val="true"  id="amount_<?= $count ?>"    data-val-required="حقل مطلوب"   data-val-regex="المدخل غير صحيح!"   class="form-control" value="0" max="<?= $row['REM_AMOUNT'] ?>" ondblclick="alert('<?= $row['REM_AMOUNT'] ?>'); return false ;">
