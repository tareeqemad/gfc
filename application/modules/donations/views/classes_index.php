...
//$MODULE_NAME= 'stores';
$TB_NAME= 'classes';
$get_url =base_url("donations/donation/public_get_classes");
...
<div class="form-body"  >
        ...
        <form  id="<?=$TB_NAME?>_form" method="get" action="<?=$get_url?>" role="form" novalidate="novalidate">
            ...
            <div class="form-group col-sm-1">
                ...
                <div>
                    ...
                    <input type="hidden"  name="did" id="txt_did" value="<?= $did ?>" class="form-control">
    ...
    <div id="container">
        <?=modules::run("donations/donation/public_get_classes", array('text'=>$text,'did'=>$did, 'id'=>$id, 'name_ar'=>$name_ar, 'name_en'=>$name_en, 'parent_id'=>$parent_id, 'grand_id'=>$grand_id ,'page'=>$page)); ?>
    </div>
    <div>
        <a class="btn-xs btn-danger" onclick="javascript:select_choose();"  href="javascript:;"><i class="icon icon-download"></i> إدراج المختار </a>
    </div>
...
{$TB_NAME}_search();
  ...
  if(parent_id==null || parent_id=='')
            parent_id= -1;
        parent.$('#h_data_search').val('/'+name_en+'/'+id+'/'+name_ar);

        get_data('{$get_url}',{did:did,id:id, name_ar:name_ar, name_en:name_en, parent_id:parent_id,grand_id:grand_id,text:text},function(data){
            $('#container').html(data);
        },'html');
    }
    ...
    destruction_percent,destruction_account_id,destruction_account_id_name,average_life_span,average_life_span_type,average_life_span_type_name,donation_file_ser,rem_amount,class_type_name,class_price_no_vat,class_type){
       ...
       // alert('#h_donation_file_ser_{$text}');
        //  parent.$('#h_donation_file_ser_{$text}').val(donation_file_ser); //h_donation_file_ser_2
       //   alert('#h_donation_file_ser_{$text}');
      //    alert('#class_type_{$text}');
       parent.$('#class_type_{$text}').text(class_type_name);
 ...
 parent.$('#{$text}').closest('tr').find('input[name="h_class_price_no_vat[]"]').val(class_price_no_vat);
 parent.$('#{$text}').closest('tr').find('input[name="donation_file_ser[]"]').val(donation_file_ser);
parent.$('#{$text}').closest('tr').find('select[name="class_type[]"]').select2("val",class_type);
    parent.$('#{$text}').closest('tr').find('input[name="amount[]"]').dblclick(function(e){

         alert(rem_amount);

        });
  parent.$('#{$text}').closest('tr').find('input[name="amount[]"]').attr("max",rem_amount);
 ...
 function select_choose(){
    ...
    ,obj.AVERAGE_LIFE_SPAN_TYPE,obj.AVERAGE_LIFE_SPAN_TYPE_NAME,obj.DONATION_FILE_SER,obj.REM_AMOUNT,obj.CLASS_TYPE_NAME);
            }else{
    ...
    obj.DESTRUCTION_PERCENT,obj.DESTRUCTION_ACCOUNT_ID,obj.DESTRUCTION_ACCOUNT_ID_NAME,obj.AVERAGE_LIFE_SPAN
    ,obj.AVERAGE_LIFE_SPAN_TYPE,obj.AVERAGE_LIFE_SPAN_TYPE_NAME,obj.DONATION_FILE_SER,obj.REM_AMOUNT,obj.CLASS_TYPE_NAME,obj.CLASS_PRICE_NO_VAT);
            }
        });
