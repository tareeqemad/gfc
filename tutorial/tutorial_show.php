 ...
 */
    ...
    $video_name= $_GET['vm'];
    $out_file=fopen("visits.txt","ab");
    $txt= '#'.date('YmdHi').'-'.$_SERVER['REMOTE_ADDR'].'-'.$video_name.'#'.PHP_EOL;
    fwrite($out_file, $txt);
    fclose($out_file);
...
<head>
    ...
    <title>شروحات النظام الموحد</title>
...
<script type="text/javascript">
    (function (window) {
        function setup(TSC) {
            TSC.playerConfiguration.addMediaSrc("videos/<?=$video_name?>.mp4");
            ...
            TSC.playerConfiguration.setPosterImageSrc("First_Frame.jpg");
